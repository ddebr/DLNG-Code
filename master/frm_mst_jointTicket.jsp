				...
				<div class="input-group">
					<div id="radioBtn" class="btn-group">
						<a <%if(selForm_id.equals(mform_id.elementAt(i))){ %> 
						 	...
						 	<%}
						System.out.println("mapplication_path.elementAt(i)****"+mapplication_path.elementAt(i));
						%>
					 	 	
					 	 	data-toggle="fun" data-title="Y" name="oper_sel" value="R" onclick="openSubMod('<%=mapplication_path.elementAt(i)%>','../master/frm_mst_jointTicket.jsp','<%=selModule1%>','<%=mform_id.elementAt(i)%>');"> <%=mform_name.elementAt(i) %></a>
					</div>
				</div>
			</div>
		<%}%>
