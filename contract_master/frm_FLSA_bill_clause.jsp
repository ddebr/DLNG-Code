...
function doSubmit(fgsa_cd,fgsa_rev_no)
{
	...
	document.forms[0].option.value="BillingDetail";
	...
	if(flag1)
	{
		var a = confirm("Do You Want To Submit Data Regarding This FGSA Bill Clause ???");	
		if(a)
		{
...
function setvalues(Freq,Due_Date,Int_Cal_Cd ,Int_Cal_Sign,Int_Cal_Per,Exchng_Rate_Cd,Exchng_Rate_Cal,Invoice_Cur_Cd,Payment_Cur_Cd,Tax_Str_CD,Exchg_Rate_Note, C_flag)
{
	...
	document.forms[0].exchg_rate_note.value=Exchg_Rate_Note;//alert(C_flag);
...
<jsp:useBean class="com.seipl.hazira.dlng.contract_master.DataBean_Contract_MasterV2" id=" masOthers" scope="page"/>   
...
<%
	...
	String FGSA_Rev_No=request.getParameter("Rev_No")==null?"":request.getParameter("Rev_No");
	...
	masOthers.setCallFlag("Billing_Dtl");
	masOthers.init();
...
<div class="tab-content">
	<div class="tab-pane active" id="">
		<div class="box mb-0">
			...
			<input name="option" type="hidden" value="BillingDetail">
			<input name="signa" type="hidden" value="">
			<input name="fgsa_cd" type="hidden" value="">
			<input name="fgsa_rev_no" type="hidden" value="">
			<input name="bscode" type="hidden" value=<%=bscode%>>
			<input name="TAX_STR_CD" type="hidden" value="0">
					...
					<div class="row">
						<div class="table-responsive col-lg-12">
							<table class="table table-striped" >
								<thead>   
									<tr><th colspan="7" class="info text-center"><b><%=cust_abr%>[<%=FGSA_No %>-<%=FGSA_Rev_No %>]: BILLING DETAILS</b></th></tr>
									<tr>
									...
									<tr>	
										<td colspan="2" class="info">Plant</td>
									    <td colspan="2" class="info">Tax Structure</td>
									    <td colspan="3" class="info">Structure Details</td>
									</tr>
									<%  if(TAX_STR_CD.size()>=0)
											{ 
												...
												{ 
										%>			<tr >	
														<td colspan="2" class="text-left"><%=(String)TAX_Plant_Nm.elementAt(i)%></td>
													    <td colspan="2" class="text-left"><%=(String)TAX_STR_CD.elementAt(i)%><input name="TAX_STR_CD" type="hidden" value="<%=(String)TAX_STR_CD.elementAt(i)%>"></td>
													    <td colspan="3" class="text-left"><%=(String)TAX_Remarks.elementAt(i)%></td>				   
													</tr>
										<%		}
										    } 
										%>
										<tr>
								    		<td align="center" colspan="7"  style="background-color: white;" class="text-center">
							    				...
							    				<%	if(write_permission.trim().equalsIgnoreCase("Y")){	%>
							    					<button type="button"  class="btn btn-success" onClick="doSubmit('<%=FGSA_No%>','<%=FGSA_Rev_No%>');">Submit </button>
