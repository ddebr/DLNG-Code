				...
				<div class="input-group">
					<div id="radioBtn" class="btn-group">
						...
						%> 
						 	...
						 	<%}%>
					 	 	data-toggle="fun" data-title="Y" name="oper_sel" value="R" onclick="openSubMod('<%=mapplication_path.elementAt(i)%>','../contract_master/frm_mst_FLSA.jsp','<%=selModule1%>','<%=mform_id.elementAt(i)%>','<%=alw_add.elementAt(i)%>','<%=alw_del.elementAt(i)%>','<%=alw_upd.elementAt(i)%>','<%=alw_view.elementAt(i)%>','<%=alw_print.elementAt(i)%>');"> <%=mform_name.elementAt(i) %></a>
					</div>
				</div>
			</div>
			<%
...
<script type="text/javascript" src="../js/bootstrap.min.js"></script>
<%-- <%System.out.println("subModUrl----"+subModUrl+"hiren"); %> --%>
