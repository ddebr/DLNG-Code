...
public class DataBean_MgmtV2_Live {
			...
			try 
			{
				...
				double total_scm = 0;
					...
					/*queryString1 = "SELECT CUSTOMER_CD,FLSA_NO,FLSA_REV_NO,SN_NO,SN_REV_NO,NVL(A.dcq,'0'),NVL(A.mdcq_percentage,'100'),NVL(A.tcq,'0'),NVL(A.variation_qty,'0'),NVL(A.DCQ_MT,'0') "
						...
						queryString1 = "SELECT CUSTOMER_CD,FLSA_NO,FLSA_REV_NO,SN_NO,SN_REV_NO,NVL(A.dcq,'0'),NVL(A.mdcq_percentage,'100'),NVL(A.tcq,'0'),NVL(A.variation_qty,'0'),NVL(A.DCQ_MT,'0') "
							+ " ,A.FCC_FLAG, SN_BASE,TO_CHAR(START_DT,'dd/mm/yyyy'),TO_CHAR(END_DT,'dd/mm/yyyy'),SN_CLOSURE_CLOSE,SN_CLOSURE_REQUEST "
...
//								//System.out.println("DAILY-NOM:   = "+queryString5);
								...
								if(rset5.next())
								{
									...
									double truck_qty = 0;
									String checkSQl="select truck_nm from DLNG_DAILY_TRUCK_NOM_DTL A "
...
*/					queryString1 = "SELECT CUSTOMER_CD,TENDER_NO,LOA_NO,LOA_REV_NO,NVL(A.dcq,'0'),NVL(A.mdcq_percentage,'100'),NVL(A.tcq,'0'),NVL(A.variation_qty,'0'),NVL(A.DCQ_MT,'0') "
						...
						+ " LOA_CLOSURE_CLOSE,LOA_CLOSURE_REQUEST "
...
//					System.out.println("dailyAllocCnt******"+dailyAllocCnt);
				...
				tot_inv_scm = nf5.format(Math.round((total_all_inv_qty*multiplying_factor)/(Double.parseDouble(ncv_gcv+""))));
...
//											System.out.println("final_qty_mmbtu_sch***"+final_qty_mmbtu_sch);
			...
			try {
				...
				/*	queryString = "SELECT TRUCK_ID,TRUCK_NM,NVL(TANK_VOL_M3,'0'),TRANS_CD FROM DLNG_TANK_TRUCK_MST "
					...
					while(rset.next()) {
						...
						String query = "SELECT TRUCK_VOL,TRUCK_ENE,NEXT_AVAIL_DAYS FROM DLNG_DAILY_TRUCK_SCH_DTL A WHERE "
								+ " A.SCH_DT=TO_DATE('"+gas_date+"','DD/MM/YYYY') "
								+ " AND A.TRUCK_NM = '"+rset.getString(2)+"' "
								+ " AND A.MAPPING_ID like '"+temp_map_id+"' "
								+ " AND A.SCH_ID like '"+temp_sch_id+"' "
								+ " AND A.rev_no = (select max(B.rev_no) from DLNG_DAILY_TRUCK_SCH_DTL B"
...
//						////System.out.println("prevTruckData****"+prevTruckData);
		...
		String tax_rate = "0";
		@SuppressWarnings("unchecked")
		private void fetchAvailableTruckDetails() {
...
//			System.out.println("balance_adv_amt--"+nf.format(balance_adv_amt));
			return nf.format(balance_adv_amt);
		}
