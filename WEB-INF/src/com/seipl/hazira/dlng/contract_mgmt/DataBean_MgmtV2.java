...
public class DataBean_MgmtV2 {
		...
		public String cancel_truck_list = "";
...
//								//System.out.println("DAILY-NOM:   = "+queryString5);
								...
								if(rset5.next())
								{
									...
									double truck_qty = 0;double tot_nom_qty = 0 ;
									String checkSQl="select truck_nm,nvl(truck_vol,0) from DLNG_DAILY_TRUCK_NOM_DTL A "
											+ " where A.NOM_DT=TO_DATE('"+gas_date+"','DD/MM/YYYY')"
											+ " AND A.NOM_ID like '"+temp_map_id+"-"+rset1.getString(4)+"-"+rset2.getString(1)+"' "
											+ " AND A.CONTRACT_TYPE='S' "
											+ " and A.rev_no = (select max(B.rev_no) from DLNG_DAILY_TRUCK_NOM_DTL B"
											+ " where B.nom_id = A.nom_id "
											+ " AND B.nom_dt = A.nom_dt"
											+ " AND B.TRUCK_NM = A.TRUCK_NM"
...
//											+ " AND REV_NO = '"+rset5.getString(7)+"' ";
									...
									while (rset6.next()) {
										nom_cnt++;
										tot_nom_qty+=rset6.getDouble(2);
										
										String invMstSql = "select nvl(TOTAL_QTY,0) from DLNG_INVOICE_MST where "
...
//										//System.out.println("invMstSql-----"+invMstSql);
									...
									double nom_qty_scm = Math.round((tot_nom_qty*multiplying_factor)/(Double.parseDouble(ncv_gcv+"")));

									daily_Buyer_Nom_Qty_Mmbtu.add(tot_nom_qty);
									daily_Buyer_Nom_Qty_Scm.add(nom_qty_scm);
									
								/////////////////////////	
									total_mmbtu += tot_nom_qty;//Double.parseDouble(rset5.getString(5));
									total_scm += nom_qty_scm;//Double.parseDouble(rset5.getString(6));
...
//												+ " AND REV_NO = '"+rset5.getString(7)+"' ";
										...
										double nom_qty_mmbtu = tot_nom_qty;//Double.parseDouble(rset5.getString(5)=="0"?"":rset5.getString(5));
										double final_qty_mmbtu = nom_qty_mmbtu - truck_qty ; 
...
//				//System.out.println("dailyNomiTrucknCnt***"+dailyNomiTrucknCnt);
				...
				if(dailyNomiTrucknCnt > 0) {
					...
					queryString="SELECT A.TRUCK_NM,A.TRUCK_ID,A.TANK_VOL_M3,A.TANK_VOL_TON,"
							...
							+ " AND B.NOM_DT = TO_DATE ('"+nomDt+"','DD/MM/YYYY') "
							+ " and B.rev_no = (select max(D.rev_no) from DLNG_DAILY_TRUCK_NOM_DTL D"
								+ " where D.nom_id = B.nom_id "
								+ " AND D.nom_dt = B.nom_dt"
								+ " AND D.TRUCK_NM = B.TRUCK_NM"
								+ " AND D.CONTRACT_TYPE = B.CONTRACT_TYPE "
								+ " AND D.ent_dt = (select max(C.ent_dt) from DLNG_DAILY_TRUCK_NOM_DTL C where C.nom_id = D.nom_id  AND C.nom_dt = D.nom_dt AND C.TRUCK_NM = D.TRUCK_NM AND C.CONTRACT_TYPE = D.CONTRACT_TYPE))"
							+ " AND A.TRUCK_NM = B.TRUCK_NM " ;
				}else {
...
//					buyerComment="<font color='blue' >"+rset8.getString(9)+"</font>";
					...
					if(dailyNomiTrucknCnt > 0) {
						...
						queryString= "SELECT TRUCK_VOL,TRUCK_ENE,TO_CHAR(SCH_DT,'DD/MM/YYYY'),SCH_TIME,REMARKS "
								+ " FROM DLNG_DAILY_TRUCK_SCH_DTL A WHERE MAPPING_ID like '"+temp_map_id+"'"
								+ " AND A.SCH_ID like '"+temp_nom_id+"' "
								+ " AND A.SCH_DT = TO_DATE ('"+nomDt+"','DD/MM/YYYY') AND TRUCK_NM='"+rset8.getString(1).trim()+"'"
								+ " AND A.CONTRACT_TYPE='"+contract_type+"'"
								+ " AND A.REV_NO= (select max(B.rev_no) from DLNG_DAILY_TRUCK_SCH_DTL B"
								+ " where B.SCH_ID = A.SCH_ID  AND B.SCH_DT = A.SCH_DT AND B.TRUCK_NM = A.TRUCK_NM and B.CONTRACT_TYPE=A.CONTRACT_TYPE"
								+ " and B.ent_dt = (select max(C.ent_dt) from DLNG_DAILY_TRUCK_SCH_DTL C where C.SCH_ID = B.SCH_ID  AND C.SCH_DT = B.SCH_DT AND C.TRUCK_NM = B.TRUCK_NM"
...
//					System.out.println("queryString***"+queryString);
					...
					String dailyAllocCnt = "SELECT LOADED_ENE,LOADED_SCM,LOADED_VOL,nvl(UNLOADED_ENE,0) FROM DLNG_TANK_VOL_DTL A WHERE "
							+ " SCH_ID like '"+temp_nom_id+"' "
							+ " AND TRN_NM='"+rset8.getString(1)+"'"
							+ " AND EFF_DT = TO_DATE ('"+nomDt+"','DD/MM/YYYY')"
							+ " and seq_no = (select max(seq_no) from DLNG_TANK_VOL_DTL B where B.EFF_DT = A.EFF_DT and B.SCH_ID = A.SCH_ID and B.TRN_NM = A.TRN_NM)";
//						System.out.println("dailyAllocCnt******"+dailyAllocCnt);
					rset10 = stmt10.executeQuery(dailyAllocCnt);
					if(rset10.next()) {
						
						alloc_cnt++;  //= Integer.parseInt(rset.getString(1)==null?"0":rset.getString(1));
						Vtruck_alloc_mmbtu.add(rset10.getString(1)==null?"0":rset10.getString(1));
						Vtruck_alloc_scm.add(rset10.getString(2)==null?"0":rset10.getString(2));
						Vtruck_alloc_mt.add(rset10.getString(3)==null?"0":rset10.getString(3));
						double unloaded_mmbtu = rset10.getDouble(4);
						
						if(invQty <= 0 ) {
							tot_tlu_mmbtu+=unloaded_mmbtu;
						}
					}else {
						alloc_cnt = 0 ;
...
//					System.out.println("dailyAllocCnt******"+dailyAllocCnt);
				...
				double final_loaded_inv_mmbtu = total_all_inv_qty+tot_tlu_mmbtu;
				tot_inv_mmbtu = nf5.format(final_loaded_inv_mmbtu);
				tot_inv_scm = nf5.format(Math.round((final_loaded_inv_mmbtu*multiplying_factor)/(Double.parseDouble(ncv_gcv+""))));
				tot_inv_mt = nf5.format((final_loaded_inv_mmbtu / convt_mmbtu_to_mt));
				...
				/*if(truckNomCnt == 0) { // if no truck selection while nomination time
					...
					while(rset8.next()) 
					{
						...
						String queryString11 = "SELECT TRUCK_VOL,TRUCK_ENE,TO_CHAR(SCH_DT,'DD/MM/YYYY'),SCH_TIME,REMARKS "
								+ " FROM DLNG_DAILY_TRUCK_SCH_DTL A"
								+ " WHERE A.MAPPING_ID like '"+temp_map_id+"'"
								+ " AND A.SCH_ID like '"+temp_nom_id+"' "
								+ " AND A.SCH_DT = TO_DATE ('"+nomDt+"','DD/MM/YYYY') AND TRUCK_NM='"+rset8.getString(1).trim()+"' "
								+ " AND A.CONTRACT_TYPE='"+contract_type+"' "
								+ " AND A.REV_NO= (select max(B.rev_no) from DLNG_DAILY_TRUCK_SCH_DTL B"
...
//						////System.out.println("SELECT: DLNG_SN_PLANT_MST: "+queryString1);
							...
							while(rset2.next())
							{
								...
								if(rset4.next())
								{
									...
									/*Hiren_20210319 *** if nominated truck's invoice generated*/
									double truck_qty = 0;double total_truck_nom_qty = 0;
										String checkSQl="select truck_nm,nvl(truck_vol,0) from DLNG_DAILY_TRUCK_NOM_DTL A"
												+ " where A.NOM_DT=TO_DATE('"+gas_date+"','DD/MM/YYYY')"
												+ " AND A.NOM_ID like '"+temp_map_id+"-"+rset1.getString(4)+"-"+rset2.getString(1)+"' "
												+ " AND A.CONTRACT_TYPE='S' "
										...
										while (rset6.next()) {
											nom_cnt++;
											total_truck_nom_qty+=rset6.getDouble(2);
										...
										total_nom_qty+=total_truck_nom_qty;
										...
										daily_Buyer_Nom_Qty_Mmbtu.add(total_truck_nom_qty);
										daily_Buyer_Nom_Qty_Scm.add(Math.round((total_truck_nom_qty*multiplying_factor)/(Double.parseDouble(ncv_gcv+""))));
...
//									System.out.println("queryString60***"+queryString6);
									if(rset6.next())
									{
										...
										/*Hiren_20210319 *** if nominated truck's invoice generated*/
											...
											String checkSQlSch="select truck_nm,nvl(truck_vol,0) from DLNG_DAILY_TRUCK_SCH_DTL A"
													+ " where A.SCH_DT=TO_DATE('"+gas_date+"','DD/MM/YYYY')"
													+ " AND A.SCH_ID like '"+temp_map_id+"-"+rset1.getString(4)+"-"+rset2.getString(1)+"' "
													+ " AND A.CONTRACT_TYPE='S' "
													+ " and A.rev_no = (select max(B.rev_no) from DLNG_DAILY_TRUCK_SCH_DTL B"
														+ " where B.sch_id = A.sch_id "
														+ " AND B.sch_dt = A.sch_dt"
														+ " AND B.TRUCK_NM = A.TRUCK_NM"
														+ " AND B.CONTRACT_TYPE = A.CONTRACT_TYPE "
											...
											while (rset7.next()) {
												truck_qty_sch+=rset7.getDouble(2);
...
//											System.out.println("truck_qty_sch***"+truck_qty_sch);
											daily_Seller_Nom_Qty_Mmbtu.add(truck_qty_sch);
											daily_Seller_Nom_Qty_Scm.add(Math.round((truck_qty_sch*multiplying_factor)/(Double.parseDouble(ncv_gcv+""))));
											
											/*if(Integer.parseInt(rset6.getString(2)) > 1) {
												
												daily_Seller_Nom_Qty_Mmbtu.add(final_qty_mmbtu_sch);
...
//										System.out.println("queryString5--SN--"+queryString5);
				...
				if(total_nom_qty>0)
				{
					daily_Total_Mmbtu = nf.format(total_nom_qty);
				}
				else
				{
					daily_Total_Mmbtu = "";
				...
				if(total_sch_qty>0)
				{
					daily_Total_Mmbtu_Seller_Nom = nf.format(total_sch_qty);
				}
				else
				{
					daily_Total_Mmbtu_Seller_Nom = "";
		...
		Vector truck_inv_cnt = new Vector();
...
//						//System.out.println("query***"+query);
						...
						if(rset4.next()){
							...
							String allocData="select LOADED_ENE,LOADED_ENE from DLNG_TANK_VOL_DTL A where "
									...
									+ " and seq_no = (select max(seq_no) from DLNG_TANK_VOL_DTL B where B.EFF_DT = A.EFF_DT and B.SCH_ID = A.SCH_ID and B.TRN_CD = A.TRN_CD)";
...
/*					queryString = "SELECT TO_CHAR(LOAD_START_TM,'DD/MM/YYYY'),TO_CHAR(LOAD_START_TM,'HH24:MI'),TO_CHAR(LOAD_END_TM,'DD/MM/YYYY')," + 
					...
					queryString = "SELECT TO_CHAR(LOAD_START_TM,'DD/MM/YYYY'),TO_CHAR(LOAD_START_TM,'HH24:MI'),TO_CHAR(LOAD_END_TM,'DD/MM/YYYY')," + 
							...
							" FROM DLNG_TANK_VOL_DTL A WHERE "
...
//						////System.out.println("key**rset.getString(6)****"+key+"-"+rset.getString(6));
					...
					for(int j=0;j<tank_truck_id.size();j++){
						...
						String prevTruckData="select flag,TRN_CD,TO_CHAR(EFF_DT,'dd/mm/yyyy')"
							+ " from DLNG_tank_vol_dtl where seq_no=(select max(seq_no) from DLNG_tank_vol_dtl "
							+ "	where trn_cd='"+tank_truck_id.elementAt(j)+"'"
							+ " and eff_dt<TO_DATE('"+gas_date+"','DD/MM/YYYY')) ";
...
//				//System.out.println("nomId**********"+nomId+"***Cont"+selContract_Type);
				String tempMap [] = buyer_mapping_id.split("-");
				String temp_map_id =tempMap[0]+"-"+tempMap[1]+"-%-"+tempMap[3]+"-%";
				...
				queryString7 = "SELECT  TRUCK_ID,TRUCK_NM,CUSTOMER_CD,A.TANK_VOL_M3,TANK_VOL_TON, A.TRANS_CD, LOAD_CAP, NEXT_LOAD_DAY,TRANS_NAME "
						+ " FROM DLNG_TANK_TRUCK_MST  A, DLNG_TRANS_MST B "
...
//							//System.out.println("nom_rev_no***"+nom_rev_no);
							if(nom_rev_no!="") {
								...
								String temp = fetchAdvDtl(temp_nom_id,selContract_Type,gas_date,rset7.getString(2),rset7.getString(1));
...
//					System.out.println("dailyNomCnt******"+dailyNomCnt);
					...
					/*checking for truck Allocation*/
					String dailyAllocCnt = "SELECT LOADED_ENE,LOADED_SCM,LOADED_VOL FROM DLNG_TANK_VOL_DTL A WHERE "
							+ " SCH_ID like '"+temp_nom_id+"' "
...
//			String nom_net_amt = "0";
			 ...
			 * if(total_nom_qty > 0) {
			 * 
			 * String nom_gross_amt = nf.format(total_nom_qty *
			 * Double.parseDouble(Sales_Rate+"") * Double.parseDouble(exg_rate+"")); String
			 * nom_tax_amt = nf.format((Double.parseDouble(nom_gross_amt) *
			 * Double.parseDouble(tax_rate+""))/100);
			 * 
			 * nom_net_amt = nf.format(Double.parseDouble(nom_gross_amt) +
			 * (Double.parseDouble(nom_tax_amt)));
			 * 
			 * }
			 */
//			double fin_nom_qty =  total_nom_qty;
			/*if(total_nom_qty > 0) {
				fin_nom_qty = total_nom_qty - total_truck_inv_qty;
			}*/
			/*
			 * System.out.println("fin_nom_qty------"+fin_nom_qty+"---Sales_Rate---"+
			 * Sales_Rate+"-----exg_rate----"+exg_rate); String nom_gross_amt =
			 * nf.format(fin_nom_qty * Double.parseDouble(Sales_Rate+"") *
			 * Double.parseDouble(exg_rate+"")); String tax_amt =
			 * nf.format((Double.parseDouble(nom_gross_amt) *
			 * Double.parseDouble(tax_rate+""))/100);
			 * 
			 * System.out.println("fin_nom_qty------"+fin_nom_qty+
			 * "********nom_gross_amt-------------"+nom_gross_amt+"******tax_amt--------"+
			 * tax_amt);
			 * 
			 * String fin_total_inv_amt = "0",inv_paid_amt = "0",inv_unpaid_amt = "0";
			 * 
			 * String allSnQtySql = "SELECT NVL(ROUND(sum(NET_AMT_INR)+"+nom_gross_amt+"+"+
			 * tax_amt+",2),0),NVL(ROUND(sum(PAY_RECV_AMT),2),0) FROM DLNG_INVOICE_MST" +
			 * " WHERE MAPPING_ID  like '"+temp_nom_id+"'  " +
			 * " AND  CONTRACT_TYPE='"+selContract_Type+"' ";
			 * //System.out.println("allSnQtySql---"+allSnQtySql); rset2 =
			 * stmt2.executeQuery(allSnQtySql); if(rset2.next()) {
			 * 
			 * fin_total_inv_amt = rset2.getString(1)==null?"0":rset2.getString(1);
			 * inv_paid_amt = rset2.getString(2)==null?"0":rset2.getString(2); }
			 * 
			 * //System.out.println("fin_total_inv_amt--before-----"+fin_total_inv_amt+
			 * "******inv_paid_amt----"+inv_paid_amt);
			 * //System.out.println("dr_amt-----"+dr_amt+"******cr_amt-----"+cr_amt);
			 * fin_total_inv_amt =
			 * nf.format((Double.parseDouble(fin_total_inv_amt+"")+Double.parseDouble(
			 * dr_amt) - Double.parseDouble(cr_amt)) - Double.parseDouble(inv_paid_amt+""));
			 * System.out.println("fin_total_inv_amt--after-----"+fin_total_inv_amt);
			 * 
			 * String total_adv_amt = "0"; String advAmtSql =
			 * "select nvl(sum(PRICE_RATE),0) from DLNG_CONT_PRICE_DTL where" +
			 * " MAPPING_ID like '"+temp_map_id+"-"+selContract_Type+"' " +
			 * " and CURRENCY_CD = '1' " + " and FLAG = 'Y' ";
			 * //System.out.println("advAmtSql----"+advAmtSql); rset6
			 * =stmt6.executeQuery(advAmtSql); if(rset6.next()) { total_adv_amt =
			 * rset6.getString(1)==null?"0":rset6.getString(1); }
			 * //System.out.println("total_adv_amt--After-----"+total_adv_amt);
			 * if(Double.parseDouble(total_adv_amt+"") >
			 * Double.parseDouble(fin_total_inv_amt+"")) {
			 * 
			 * balance_adv_amt = nf.format(Double.parseDouble(total_adv_amt+"") -
			 * Double.parseDouble(fin_total_inv_amt+"")); }else {
			 * 
			 * balance_adv_amt = "0"; }
			 */
...
//					System.out.println("truckLoadedFlag.elementAt(i)******1**"+truckLoadedFlag.elementAt(i));
					if(truckLoadedFlag.elementAt(i).equals("N")){
						...
						fetchTruckNomData = "SELECT TRUCK_VOL,TRUCK_ENE,TO_CHAR(ARRIVAL_DT,'DD/MM/YYYY'),ARRIVAL_TIME,REMARKS,NEXT_AVAIL_DAYS "
								+ " FROM DLNG_DAILY_TRUCK_NOM_DTL WHERE MAPPING_ID like '"+temp_map_id+"'"
								+ " AND NOM_ID like '"+temp_nom_id+"' AND REV_NO='"+revNo+"'"
								+ " AND NOM_DT = TO_DATE ('"+nomDt+"','DD/MM/YYYY') AND TRUCK_NM='"+truckNameAT.elementAt(i)+"' ";
...
//				String tempMap [] = buyer_mapping_id.split("-");
				...
				cancel_truck_list = "";
				/*String loaded_truck = "SELECT TRUCK_NM,NOM_ID,nvl(TRUCK_VOL,0) FROM DLNG_DAILY_TRUCK_NOM_DTL WHERE NOM_ID like '"+temp_map_id+"'"
				...
				String loaded_truck = "SELECT distinct(TRUCK_NM),NOM_ID,nvl(TRUCK_VOL,'0'),to_char(ENT_DT,'dd/mm/yyyy')"
						+ " FROM DLNG_DAILY_TRUCK_NOM_DTL A"
...
//				System.out.println("loaded_truck***"+loaded_truck);
				...
				while (rset1.next()) {
					...
					String tmp_truck_nm = rset1.getString(1)==null?"":rset1.getString(1);
...
//							////System.out.println("Else if 2 (SN_no !=)  OnlyLoadedTruck : "+rset1.getString(1)+"..len :"+rset1.getString(1).length());
					...
					//Hiren_20230221 for checking scheduled
					String canCntSql = "select distinct(TRUCK_NM),nvl(TRUCK_VOL,0) from DLNG_DAILY_TRUCK_SCH_DTL A"
							+ " where A.truck_nm = '"+tmp_truck_nm+"'"
							+ " and A.sch_id = '"+nomId+"'"
							+ " and A.sch_dt = to_date('"+nomDt+"','dd/mm/yyyy')"
							+ " and A.rev_no = (select max(B.rev_no) from DLNG_DAILY_TRUCK_SCH_DTL B"
								+ " where B.sch_id = A.sch_id "
								+ " AND B.sch_dt = A.sch_dt"
								+ " AND B.TRUCK_NM = A.TRUCK_NM "
								+ " AND B.ent_dt = (select max(C.ent_dt) from DLNG_DAILY_TRUCK_SCH_DTL C where C.sch_id = B.sch_id  AND C.sch_dt = B.sch_dt AND C.TRUCK_NM = B.TRUCK_NM))";
//					System.out.println("canCntSql****"+canCntSql);
					rset2 = stmt2.executeQuery(canCntSql);
					if(rset2.next()) {
						double loaded_vol = rset2.getDouble(2);
						if(loaded_vol <= 0) {
//							cancel_truck_list = tmp_truck_nm+"&"+cancel_truck_list;
							cancel_truck_list = "Truck No. : "+tmp_truck_nm+"\n"+cancel_truck_list;
						}
					}else {
						String canNomSql = "select distinct(TRUCK_NM),nvl(TRUCK_VOL,0) from DLNG_DAILY_TRUCK_NOM_DTL A"
								+ " where A.truck_nm = '"+tmp_truck_nm+"'"
								+ " and A.nom_id = '"+nomId+"'"
								+ " and A.nom_dt = to_date('"+nomDt+"','dd/mm/yyyy')"
								+ " and A.rev_no = (select max(B.rev_no) from DLNG_DAILY_TRUCK_NOM_DTL B"
									+ " where B.nom_id = A.nom_id "
									+ " AND B.nom_dt = A.nom_dt"
									+ " AND B.TRUCK_NM = A.TRUCK_NM "
									+ " AND B.ent_dt = (select max(C.ent_dt) from DLNG_DAILY_TRUCK_NOM_DTL C where C.nom_id = B.nom_id  AND C.nom_dt = B.nom_dt AND C.TRUCK_NM = B.TRUCK_NM))";
//						System.out.println("canNomSql****"+canNomSql);
						rset2 = stmt2.executeQuery(canNomSql);
						if(rset2.next()) {
							cancel_truck_list = "Truck No. : "+tmp_truck_nm+"\n"+cancel_truck_list;
...
//				System.out.println("cancel_truck_list****"+cancel_truck_list);
			} catch (Exception e) {
				e.printStackTrace();
			}
					...
					if(!nomDtList.elementAt(i).equals("")) {
						...
						if(invCnt == 0) {
							...
							if(allocCnt == 0) {
								...
								String schMstSql = "select TRUCK_VOL from DLNG_DAILY_TRUCK_SCH_DTL A where "
										+ " A.SCH_ID  like '"+nom_id+"'  "
										+ " AND A.CONTRACT_TYPE='"+cont_typ+"' "
										+ " AND A.SCH_DT=TO_DATE('"+nomDtList.elementAt(i)+"','DD/MM/YYYY')"
										+ " AND A.TRUCK_NM = '"+truck_nm+"' "
										+ " and A.REV_NO = (select max(B.rev_no) from DLNG_DAILY_TRUCK_SCH_DTL B"
											+ " where B.SCH_ID = A.SCH_ID AND B.SCH_DT = A.SCH_DT AND B.TRUCK_NM = A.TRUCK_NM AND B.CONTRACT_TYPE = A.CONTRACT_TYPE"
											+ " and B.ent_dt = (select max(C.ent_dt) from DLNG_DAILY_TRUCK_SCH_DTL C where"
											+ " C.SCH_ID = B.SCH_ID AND C.SCH_DT = B.SCH_DT AND C.TRUCK_NM = B.TRUCK_NM AND C.CONTRACT_TYPE = B.CONTRACT_TYPE))";
...
//										nomRev = rset1.getInt(1);
									...
									String tot_nom_sql ="select nvl(TRUCK_VOL,0)  from DLNG_DAILY_TRUCK_NOM_DTL A where "
											+ " A.NOM_DT = TO_DATE('"+nomDtList.elementAt(i)+"','DD/MM/YYYY') "
											+ " and A.TRUCK_NM='"+truck_nm+"' "
											+ " AND A.NOM_ID  like '"+nom_id+"'"
											+ " AND A.CONTRACT_TYPE='"+cont_typ+"' "
											+ " and A.REV_NO = (select max(B.rev_no) from DLNG_DAILY_TRUCK_NOM_DTL B"
												+ " where B.NOM_ID = A.NOM_ID AND B.NOM_DT = A.NOM_DT AND B.TRUCK_NM = A.TRUCK_NM AND B.CONTRACT_TYPE = A.CONTRACT_TYPE"
												+ " and B.ent_dt = (select max(C.ent_dt) from DLNG_DAILY_TRUCK_NOM_DTL C where"
												+ " C.NOM_ID = B.NOM_ID AND C.NOM_DT = B.NOM_DT AND C.TRUCK_NM = B.TRUCK_NM AND C.CONTRACT_TYPE = B.CONTRACT_TYPE))";
...
//			int shared_cnt = 0 ;
	...
	//				System.out.println("BilledQty*****"+BilledQty);
				...
				System.out.println("total_net_amt*****"+nf.format(total_net_amt)+"****total_security****"+nf.format(total_security));
...
//--------- HA18MARCH2020 -------
		...
		public String getCancel_truck_list() {
			return cancel_truck_list;
		}
