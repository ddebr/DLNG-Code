...
//			//System.out.println("s****"+s);
	 ...
	 Vector Vseq_wise_total_amt =  new Vector();
...
//			System.out.println("fetchAdvDtlSql------------"+fetchAdvDtlSql);
			...
			while (rset.next()) {
				...
				String sseq_no = rset.getString(1)==null?"":rset.getString(1);
...
//					System.out.println("rset.getString(14)----"+rset.getString(14));
				...
				if(drcrFlg.equalsIgnoreCase("")) {
					seq_wise_total_amt+=rset.getDouble(7);
				
					sseq_no = sseq_no+".%";
					String drDataSql = "select nvl(sum(PAY_AMT),0) from "
							+ " DLNG_ADVC_PAY_MST where "
							+ " CUSTOMER_CD='"+bscode+"'"
							+ " and FLSA_NO = '"+fgsa_cd+"' "
							+ " and SN_NO = '"+sn_cd+"'  "
							+ " and CONTRACT_TYPE='"+cont_typ+"' "
							+ " and seq_no like '"+sseq_no+"'"
							+ " and DR_CR_FLAG = 'D' "
							+ " ORDER BY seq_no";
//					System.out.println("drDataSql****"+drDataSql);
					rset2 = stmt2.executeQuery(drDataSql);
					if(rset2.next()) {
						seq_wise_total_amt-=rset2.getDouble(1);
					}
					String crDataSql = "select nvl(sum(PAY_AMT),0) from "
							+ " DLNG_ADVC_PAY_MST where "
							+ " CUSTOMER_CD='"+bscode+"'"
							+ " and FLSA_NO = '"+fgsa_cd+"' "
							+ " and SN_NO = '"+sn_cd+"'  "
							...
							+ " and DR_CR_FLAG = 'C' "
							+ " ORDER BY seq_no";
//					System.out.println("crDataSql****"+crDataSql);
					rset2 = stmt2.executeQuery(crDataSql);
					if(rset2.next()) {
						seq_wise_total_amt+=rset2.getDouble(1);
					}
				}
				Vseq_wise_total_amt.add(nf.format(seq_wise_total_amt));
//				System.out.println("seq_wise_total_amt****"+nf.format(seq_wise_total_amt));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Vector getVseq_wise_total_amt() {
		return Vseq_wise_total_amt;
	}
}
