...
//			String mapping_id=""+bscode+"-"+FGSA_cd+"-%-"+SN_CD+"-"+SN_REVNo+"-S";
			for(int i=0;i<component_cd.size();i++)
			{
				queryString = "select price_rate,currency_cd,flag " +
						"FROM DLNG_CONT_PRICE_DTL WHERE mapping_id like '"+mapping_id+"' AND " +
...
boolean billFlag = false;
	public void fetch_SN_Billing_Dtl()
	{
		...
		{
			boolean flag=true;
			queryString = "select * from DLNG_SN_BILLING_DTL WHERE FLSA_NO='"+FGSA_cd+"' And FLSA_REV_NO='"+FGSA_REVNo+"' And CUSTOMER_CD='"+bscode+"' And CONT_TYPE='S' And SN_NO='"+SN_CD+"' And SN_REV_NO='"+SN_REVNo+"'";
	...
	public void fetch_LOA_Billing_Dtl()
	{
		try
		{
			...
			queryString = "select CUSTOMER_CD,FLSA_NO,FLSA_REV_NO,SN_NO,SN_REV_NO,CONT_TYPE,BILLING_FREQ,DUE_DATE," +
					...
					"PAYMENT_CUR_CD,TAX_STRUCT_CD,EXCHG_RATE_NOTE,INV_CRITERIA, FLAG from DLNG_SN_BILLING_DTL " +
