...
//import com.seipl.hazira.dlng.cargo_procurement.DataBean_Cargo_Procurement;
...
public class DataBean_Contract_Master2
{
...
//	    				Fetch_FGSA_Tender_LC_Dtl();
	    			...
	    			else if(callFlag.equals("CargoAllocationDtl2"))  //SB20200508
	    			{
	    				CheckForColumn_FMS7_MAN_CONFIRM_CARGO_DTL();
	    				Customer_DTL();
	    				fetchCargoDtl2(); //SB20200508
	    			}
	    			else if(callFlag.equalsIgnoreCase("commodity_rate_entry")) 
	    			{
	    				checkForTableFMS9_COMMODITY_VALUE_DTL();
	    				get_community_Rate();
	    			///	if(!rate_cd.trim().equalsIgnoreCase("") && !rate_cd.trim().equalsIgnoreCase("0"))
	    				{
	    				///	get_Inserted_Interest_Exchg_Rate_Value();
	    				}
	    			}
	    			else if(callFlag.equalsIgnoreCase("auditRpt_price_change")) 
	    			{
	    				auditRpt_price_change();
	    			}
	...
	public void fetch_LC_Dtl() throws Exception {
		try {
			String queryString = "select lc_amount,to_char(lc_valid_date,'dd/mm/yyyy'),bank_dtl,lc_file_nm,remark,flag,fcc_note,fcc_apply "
					+ "from fms8_lc_mst where customer_cd='"+Buyer_cd+"' and fgsa_no='"+FGSA_cd+"' "
					+ "and fgsa_rev_no='"+FGSA_REVNo+"' and sn_no='"+SN_CD+"' and sn_rev_no='"+SN_REVNo+"' and contract_type='"+cont_type+"' ";
...
/////////////////SB20200518/////////////////////////////////////////////////	
public void checkForTableFMS9_COMMODITY_VALUE_DTL() throws Exception {
try {
int count = 0;
String query="SELECT COUNT(TNAME) FROM TAB WHERE UPPER(TNAME)='FMS9_COMMODITY_VALUE_DTL'";
rset=stmt.executeQuery(query);
	if(rset.next())
	{
	count=rset.getInt(1);
			if(count==0)
			{
			query="CREATE TABLE FMS9_COMMODITY_VALUE_DTL ("
			+ "DELV_MTH DATE,"
			+ "BUSINESS_DT DATE,"
			+ "COMMODITY_VALUE NUMBER(8,4),"
			+ "CURRENCY VARCHAR2(5),"
			+ "MOU VARCHAR2(10),"
			...
			+ "APRV_DT DATE,"
			+ "FLAG CHAR(1),"
			+ "CONSTRAINT FMS9_COMMODITY_VALUE_DTL PRIMARY KEY(DELV_MTH,BUSINESS_DT))";
			System.out.println("Creaing Table.."+query);
			stmt.executeUpdate(query);
			conn.commit();
			}
...
conn.rollback();
	...
	///////////////////////SB20200407: To find out BILLING FREQ/////////////////////////
				...
				/*SB20200329: showing twicw same Cargo 			queryString = "SELECT CARGO_REF_NO,MARGIN,TOTAL_MARGIN,AVG_MARGIN,SALE_PRICE,ALLOC_QTY FROM "
					...
					/*SB20200329		queryString = "SELECT CARGO_REF_NO,MARGIN,TOTAL_MARGIN,AVG_MARGIN,SALE_PRICE,ALLOC_QTY FROM "
						...
						System.out.println("STEP-3:SN..."+queryString);
							...
							if(countData==0) {
								queryString = "SELECT ALLOC_QTY,SALE_PRICE,COST_PRICE, CARGO_REF_NO FROM "
										+ "FMS7_DUMMY_CARGO_DTL WHERE CUSTOMER_CD='"+customer_cd_+"' AND FGSA_NO='"+rset.getString(1)+"' "
										+ "AND SN_NO='"+rset.getString(3)+"' AND CONTRACT_TYPE='"+contract_type+"'";
								System.out.println("STEP-4:SN: FMS7_DUMMY_CARGO_DTL:"+queryString);
								rset2 = stmt2.executeQuery(queryString);
								while(rset2.next()) {
									countData++;
								//SB20200424	temp_ref_no.add("0");
									temp_ref_no.add(rset2.getString(4)==null?"0":rset2.getString(4)); //SB20200424
									temp_margin.add(rset2.getString(2)==null?"0.00":nfNew1.format(Double.parseDouble(rset2.getString(2))));
								...
								queryString = "SELECT ALLOC_QTY,SALE_PRICE,COST_PRICE, CARGO_REF_NO FROM "
										+ "FMS8_OWN_CARGO_DTL WHERE CUSTOMER_CD='"+customer_cd_+"' AND FGSA_NO='"+rset.getString(1)+"' "
										+ "AND SN_NO='"+rset.getString(3)+"' AND CONTRACT_TYPE='"+contract_type+"' ";
								System.out.println("STEP-4A:SN..."+queryString);
								rset2 = stmt2.executeQuery(queryString);
								while(rset2.next()) {
									countData++;
								//SB20200424	temp_ref_no.add("0");
							...
							/*SB20200326	queryString = "SELECT NVL(MODIFICATION_SEQ_NO,'0'),NVL(FLAG,''),ENT_BY,APPROVE_BY,"
								...
								if(rset2.next()) {
									...
									for(int k=0;k<temp_ref_no.size();k++) {
										...
										System.out.println("STEP-5A:SN: "+queryString);
					...
					/*SB20200329		queryString = "SELECT CARGO_REF_NO,MARGIN,TOTAL_MARGIN,AVG_MARGIN,SALE_PRICE,ALLOC_QTY FROM "
							...
							if(countData==0) {
								queryString = "SELECT ALLOC_QTY,SALE_PRICE,COST_PRICE, CARGO_REF_NO FROM "
										+ "FMS7_DUMMY_CARGO_DTL WHERE CUSTOMER_CD='"+customer_cd_+"' AND FGSA_NO='"+rset.getString(1)+"' "
										+ "AND SN_NO='"+rset.getString(3)+"' AND CONTRACT_TYPE='"+contract_type+"'";
								System.out.println("STEP-4:LOA: FMS7_DUMMY_CARGO_DTL: "+queryString);
								rset2 = stmt2.executeQuery(queryString);
								while(rset2.next()) {
									//SB20200424 temp_ref_no.add("0");
									temp_ref_no.add(rset2.getString(4)==null?"0":rset2.getString(4));//SB20200424
								...
								queryString = "SELECT ALLOC_QTY,SALE_PRICE,COST_PRICE, CARGO_REF_NO FROM "
										+ "FMS7_DUMMY_CARGO_DTL WHERE CUSTOMER_CD='"+customer_cd_+"' AND FGSA_NO='"+rset.getString(1)+"' "
										+ "AND SN_NO='"+rset.getString(3)+"' AND CONTRACT_TYPE='"+contract_type+"' ";
								System.out.println("STEP-4A:LOA: FMS7_DUMMY_CARGO_DTL: "+queryString);
								rset2 = stmt2.executeQuery(queryString);
								while(rset2.next()) {
									//SB20200424 temp_ref_no.add("0");
									temp_ref_no.add(rset2.getString(4)==null?"0":rset2.getString(4));//SB20200424
							...
							queryString = "SELECT NVL(MODIFICATION_SEQ_NO,'0'),NVL(FLAG,''),ENT_BY,APPROVE_BY,"
									...
									+ "FGSA_NO='"+rset.getString(1)+"' AND SN_NO = '"+rset.getString(3)+"' AND CUSTOMER_CD = '"+customer_cd_+"' AND "
									+ "CONTRACT_TYPE='"+contract_type+"' ORDER BY MODIFICATION_SEQ_NO DESC";
							System.out.println("STEP-5:LOA: FMS8_CARGO_ALLOC_REVISED_DTL: "+queryString);
							rset2 = stmt2.executeQuery(queryString);
							if(rset2.next()) {
								int seq_no = rset2.getInt(1);
								CAR_MAX_MODIFY_SEQ_NO.add(seq_no);
								...
								for(int k=0;k<temp_ref_no.size();k++) {
									queryString = "SELECT NEW_SALE_PRICE,NEW_MARGIN,NEW_AVG_MARGIN,NEW_TOT_MARGIN "
											...
											+ "CONTRACT_TYPE='"+contract_type+"' AND MODIFICATION_SEQ_NO='"+rset2.getInt(1)+"' "
											+ "AND FLAG='"+rset2.getString(2)+"' AND CARGO_REF_NO='"+temp_ref_no.elementAt(k)+"' AND FLAG!='A'";
									System.out.println("STEP-5A:LOA: FMS8_CARGO_ALLOC_REVISED_DTL: "+queryString);
									rset3 = stmt3.executeQuery(queryString);
									if(rset3.next()) {
										temp_new_sale_price.add(k,rset3.getString(1)==null?"0.00":rset3.getString(1));
										temp_new_margin.add(k,rset3.getString(2)==null?"0.00":rset3.getString(2));
			...
			System.out.println("CARGO-SIZE  :"+CAR_SN_NO.size());
			...
			System.out.println("STEP-6: FMS8_CARGO_ALLOC_REVISED_DTL: "+queryString);
			rset = stmt.executeQuery(queryString);
			while(rset.next()) {
				String flag = rset.getString(1)==null?"A":rset.getString(1);
				String effDt = rset.getString(3)==null?"":rset.getString(3);
...
//			System.out.println("FGSA Governing Clauses Query = "+queryString);			
	...
	public void fetch_LOA_transporter_details()
	{
		try
		{
			queryString= "Select distinct(A.TRANSPORTER_CD) , B.TRANSPORTER_ABBR  from FMS7_LOA_TRANSPORTER_MST A , " +
...
//			double hlpl_own_volume = Double.parseDouble(available_qty);
	...
	//////////////////////////////////////////SB20200508: New function for Changing Cargo Price ///////////////////////////////
	public void fetchCargoDtl2() throws Exception {
		try {
			//Fetching Contracts which are not still started....
			Vector VCAR_REF_NO_temp= new Vector();
			Vector VCAR_CONF_ENG_temp= new Vector();
			Vector VCAR_CONF_ENG_UNIT_temp= new Vector();
			Vector VCAR_CONF_PRICE_temp= new Vector();
			System.out.println(month+" :STEP-0:CARGO: "+year);
			String SearchDtFrom ="01/"+month+"/"+year;//"01/01/2020";
			System.out.println(month+" :STEP-0:CARGO: "+SearchDtFrom);
			String SearchDtTo ="";
			int SN_LOA_Count=0;
			 queryString = "select TO_CHAR(LAST_DAY(to_date('"+SearchDtFrom+"','dd/mm/yyyy')),'dd/mm/yyyy') from dual";
			  rset = stmt.executeQuery(queryString);
			  if(rset.next()) {
				   SearchDtTo = rset.getString(1);
			  }
			
				String NextNewPriceDt =""; String NextNomMth="01"; String MaxNomDt ="";
			queryString = "SELECT E.PRICE_TYPE, A.CARGO_REF_CD, TO_CHAR(A.DELV_FROM_DT,'DD/MM/YYYY'), TO_CHAR(A.DELV_TO_DT,'DD/MM/YYYY'), A.CONFIRM_VOL,C.UNIT_ABR, "+
					" A.PRICE "+
					" FROM FMS7_MAN_CONFIRM_CARGO_DTL A, FMS7_UNIT_MST C,  FMS7_MAN_REQ_MST E WHERE "+
					" A.DELV_FROM_DT BETWEEN TO_DATE('"+SearchDtFrom+"','DD/MM/YYYY') AND TO_DATE('"+SearchDtTo+"','DD/MM/YYYY') AND E.MAN_CD=A.MAN_CD AND "+ 
					" A.UNIT_CD=C.UNIT_CD AND A.CARGO_STATUS='T' "+
				//SB20200524: NR "AND CARGO_REF_CD IN (SELECT CARGO_REF_NO FROM FMS7_SN_CARGO_DTL) "+  //Added to filter only linked Cargo
					" ORDER BY A.MAN_CONF_CD, A.CARGO_REF_CD";
				//	System.out.println("STEP-0:CARGO: "+queryString);
					rset = stmt.executeQuery(queryString);
					while(rset.next()) {
					//	VCAR_PRICE_TYPE.add(rset.getString(1)==null?"0":rset.getString(1));
						VCAR_CONF_SEQ.add(rset.getString(2)==null?"0":rset.getString(2));
						VCAR_REF_NO_temp.add(rset.getString(2)==null?"0":rset.getString(2));
					//	VCAR_DELV_FM.add(rset.getString(3)==null?"":rset.getString(3));
					//	VCAR_DELV_TO.add(rset.getString(4)==null?"":rset.getString(4));
						VCAR_CONF_ENG_temp.add(rset.getString(5)==null?"":rset.getString(5));
						VCAR_CONF_ENG_UNIT_temp.add(rset.getString(6)==null?"":rset.getString(6));			
					//	VCAR_PRICE_FM.add(rset.getString(7)==null?"":rset.getString(7));					
					//	VCAR_PRICE_TO.add(rset.getString(8)==null?"":rset.getString(8));
						VCAR_CONF_PRICE_temp.add(rset.getString(7)==null?"0":rset.getString(7));
					}
					for(int i=0; i<VCAR_REF_NO_temp.size(); i++)
					{
						SN_LOA_Count=0;
						String MaxNewPriceDt=""; String MaxNewSalePrice=""; String MaxNewOriSalePrice=""; String MaxNewPriceFlag="";contract_type="O"; String SNLOASalePrice=""; 
						String PriceType="";
						queryString1 = "SELECT TO_CHAR(MAX(NEW_PRICE_EFF_DT),'DD/MM/YYYY')"+
								" FROM FMS8_CARGO_ALLOC_REVISED_DTL WHERE CARGO_REF_NO='"+VCAR_REF_NO_temp.elementAt(i)+"' AND CONTRACT_TYPE IN ('O','G','X') ";
					//		System.out.println("STEP-1B:FMS8_CARGO_ALLOC_REVISED_DTL "+queryString1);
							rset1 = stmt.executeQuery(queryString1);
							while(rset1.next()) {
								MaxNewPriceDt=(rset1.getString(1)==null?"":rset1.getString(1));
							}
						if(!MaxNewPriceDt.equals(""))
						{
						queryString1 = "SELECT TO_CHAR(MAX(NEW_PRICE_EFF_DT),'DD/MM/YYYY'), NEW_SALE_PRICE, ORI_SALE_PRICE, FLAG "+
								" FROM FMS8_CARGO_ALLOC_REVISED_DTL WHERE CARGO_REF_NO='"+VCAR_REF_NO_temp.elementAt(i)+"' AND CONTRACT_TYPE IN ('O','G','X') AND NEW_PRICE_EFF_DT =TO_DATE('"+MaxNewPriceDt+"','DD/MM/YYYY') "
							//	+ "AND FGSA_NO='"+rset.getString(1)+"' AND FGSA_REV_NO='"+rset.getString(2)+"' AND SN_NO='"+rset.getString(3)+"' AND SN_REV_NO='"+rset.getString(4)+"' AND CUSTOMER_CD='"+rset.getString(14)+"' "
								+ " GROUP BY NEW_PRICE_EFF_DT, NEW_SALE_PRICE, ORI_SALE_PRICE, FLAG ORDER BY FLAG ASC";
							System.out.println("STEP-1B:FMS8_CARGO_ALLOC_REVISED_DTL "+queryString1);
							rset1 = stmt.executeQuery(queryString1);
							if(rset1.next()) {
								MaxNewPriceDt=(rset1.getString(1)==null?"":rset1.getString(1));
								MaxNewSalePrice=(rset1.getString(2));
								MaxNewSalePrice=nf2.format(Double.parseDouble(MaxNewSalePrice));
								MaxNewOriSalePrice=(rset1.getString(3));
								MaxNewPriceFlag=(rset1.getString(4));
							}
						}
							//System.out.println("STEP "+MaxNewPriceFlag);
						queryString = "SELECT E.PRICE_TYPE, A.CARGO_REF_CD, TO_CHAR(A.DELV_FROM_DT,'DD/MM/YYYY'), TO_CHAR(A.DELV_TO_DT,'DD/MM/YYYY'), A.CONFIRM_VOL,C.UNIT_ABR, "+
								" A.PRICE, A.PRICE_TO, A.REMARK "+
								" FROM FMS7_MAN_CONFIRM_CARGO_DTL A, FMS7_UNIT_MST C,  FMS7_MAN_REQ_MST E WHERE "+
								" A.DELV_FROM_DT BETWEEN TO_DATE('"+SearchDtFrom+"','DD/MM/YYYY') AND TO_DATE('"+SearchDtTo+"','DD/MM/YYYY') AND E.MAN_CD=A.MAN_CD AND "+ 
						...
						//		System.out.println("STEP-0:CARGO: "+queryString);
								rset = stmt.executeQuery(queryString);
								while(rset.next()) {
									if(rset.getString(1).equals("F")) PriceType="FIXED PRICE";
									else if(rset.getString(1).equals("R")) PriceType="PRICE RANGE";
									else if(rset.getString(1).equals("M")) PriceType="FORMULA";
									VCAR_PRICE_TYPE.add(PriceType);
									VCAR_REF_NO.add(VCAR_REF_NO_temp.elementAt(i));
							VCAR_DELV_FM.add(rset.getString(3)==null?"":rset.getString(3));
							VCAR_DELV_TO.add(rset.getString(4)==null?"":rset.getString(4));
							VCAR_CONF_ENG.add(VCAR_CONF_ENG_temp.elementAt(i));
							VCAR_CONF_ENG_UNIT.add(VCAR_CONF_ENG_UNIT_temp.elementAt(i));			
							//VCAR_PRICE_FM.add(rset.getString(7)==null?"":rset.getString(7));
							String val=rset.getString(8)==null?"0":rset.getString(8);
							if(MaxNewPriceFlag.equals("R")||MaxNewPriceFlag.equals(""))
							{
								VCAR_PRICE_FM.add(nf2.format(Double.parseDouble(rset.getString(7)==null?"0":rset.getString(7))));
								VCAR_PRICE_TO.add(nf2.format(Double.parseDouble(rset.getString(8)==null?"0":rset.getString(8))));
							}
							else
							{
								VCAR_PRICE_FM.add(nf2.format(Double.parseDouble(rset.getString(8)==null?"0":rset.getString(8))));
								VCAR_PRICE_TO.add(nf2.format(Double.parseDouble(rset.getString(7)==null?"0":rset.getString(7))));
							}
							VCAR_PRICE_REMARK.add(rset.getString(9)==null?"":rset.getString(9));
							VCAR_CONF_PRICE.add(nf2.format(Double.parseDouble(""+VCAR_CONF_PRICE_temp.elementAt(i))));
							VCONT_FGSA_NO.add("0");	
							VCONT_FGSA_REV_NO.add("0");		
							VCONT_SN_NO.add("0");
							VCONT_START_DT.add(rset.getString(3)==null?"":rset.getString(3));					
							VCONT_END_DT.add(rset.getString(4)==null?"":rset.getString(4));
							
							VCAR_ALLOC_QTY.add(rset.getString(5)==null?"":rset.getString(5));
							VCONT_SALE_PRICE.add(MaxNewSalePrice);  //New Price
							VCONT_COST_PRICE.add(rset.getString(7)); //Price Not yet changed
							VCONT_MARGIN.add("");
							VCONT_AVG_MARGIN.add("");
							VCONT_TOT_MARGIN.add("");
							VCONT_CUST_CD.add("0");
							VPRICE_REQ_FLAG.add("Y"); //SB20200327
							VPRICE_APRV_FLAG.add(MaxNewPriceFlag); //SB20200327
							VContType.add("X"); //SB20200327
						//	VMaxNomDt.add("N.A."); //SB20200326
							VNextNewPriceDt.add(MaxNewPriceDt); //SB20200326
							VCONT_NEW_SALE_PRICE.add(""); //SB20200326
							VCONT_NEW_MARGIN.add(""); //SB20200326
							
								}
					queryString = "SELECT B.FGSA_NO,B.FGSA_REV_NO,B.SN_NO,B.SN_REV_NO,TO_CHAR(SIGNING_DT,'DD/MM/YYYY'),"
							+ "TO_CHAR(START_DT,'DD/MM/YYYY'),TO_CHAR(END_DT,'DD/MM/YYYY'),ALLOC_QTY, SALE_PRICE, COST_PRICE, MARGIN, AVG_MARGIN,TOTAL_MARGIN,B.CUSTOMER_CD "
							+ ", AUTH_BY, APRV_BY, B.STATUS"+
							" FROM FMS7_SN_CARGO_DTL B, FMS7_SN_MST C WHERE B.CARGO_REF_NO='"+VCAR_REF_NO_temp.elementAt(i)+"' "//DELV_TO_DT >=TO_DATE('"+SearchDt+"','DD/MM/YYYY') "
			...
			//	System.out.println("STEP-1:SN "+queryString);
				...
				while(rset3.next()) {
					...
					VCAR_PRICE_FM.add("");
					VCAR_PRICE_TO.add("");
					VCAR_PRICE_REMARK.add("");
					VCAR_REF_NO.add(VCAR_REF_NO_temp.elementAt(i));
			
					VCAR_CONF_ENG.add(VCAR_CONF_ENG_temp.elementAt(i));
					VCAR_CONF_ENG_UNIT.add(VCAR_CONF_ENG_UNIT_temp.elementAt(i));			
				
					VCAR_CONF_PRICE.add(VCAR_CONF_PRICE_temp.elementAt(i));
					VCONT_FGSA_NO.add("FGSA-"+rset3.getString(1));	
					VCONT_FGSA_REV_NO.add(rset3.getString(2)==null?"":rset3.getString(2));		
					VCONT_SN_NO.add("SN-"+rset3.getString(3));
					VCONT_START_DT.add(rset3.getString(6)==null?"":rset3.getString(6));					
					VCONT_END_DT.add(rset3.getString(7)==null?"":rset3.getString(7));
					
					VCAR_ALLOC_QTY.add(rset3.getString(8)==null?"":rset3.getString(8));
					VCONT_SALE_PRICE.add(nf2.format(Double.parseDouble(rset3.getString(9))));
					SNLOASalePrice=rset3.getString(9);
					VCONT_COST_PRICE.add(nf2.format(Double.parseDouble(rset3.getString(10))));
					VCONT_MARGIN.add(nf2.format(Double.parseDouble(rset3.getString(11)==null?"0":rset3.getString(11))));//rset.getString(11)==null?"":rset.getString(11));
					VCONT_AVG_MARGIN.add(rset3.getString(12)==null?"0":rset3.getString(12));
					VCONT_TOT_MARGIN.add(rset3.getString(13)==null?"0":rset3.getString(13));
					VCONT_CUST_CD.add(rset3.getString(14)==null?"0":rset3.getString(14));
					VPRICE_REQ_FLAG.add(rset3.getString(15)==null?"":rset3.getString(15)); //SB20200327
					VPRICE_APRV_FLAG.add(rset3.getString(17)==null?"":rset3.getString(17)); //SB20200327
					
					customer_cd=rset3.getString(14)==null?"0":rset3.getString(14);
					VContType.add("S"); //SB20200327
			//		VMaxNomDt.add("N.A."); //SB20200326
					VNextNewPriceDt.add(MaxNewPriceDt); //SB20200326
					VCONT_NEW_SALE_PRICE.add(MaxNewSalePrice); //SB20200326
					VCONT_NEW_MARGIN.add(""); //SB20200326
					VPrice_Change_Dtl.add(VCAR_REF_NO_temp.elementAt(i));
					VCAR_SNLOA_CNT.add(""); 
				//	System.out.println("VCONT_FGSA_NO: "+rset3.getString(1));	
					//////////////////////////SN -Cargo Price Dtl///////////////////////////////////////
					String fgsaNo=""+rset3.getString(1); String fgsaRevNo=""+rset3.getString(2); String snNo=""+rset3.getString(3); String snRevNo=""+rset3.getString(4); String customerCd=""+customer_cd;
					queryString = "SELECT FGSA_NO,FGSA_REV_NO,SN_NO,SN_REV_NO,ALLOC_QTY, NEW_SALE_PRICE, ORI_SALE_PRICE, ORI_MARGIN,  ORI_AVG_MARGIN,ORI_TOT_MARGIN, CUSTOMER_CD, FLAG, TO_CHAR(NEW_PRICE_EFF_DT,'DD/MM/YYYY'),NEW_SALE_PRICE, NEw_MARGIN "
							+ "FROM FMS8_CARGO_ALLOC_REVISED_DTL WHERE CARGO_REF_NO='"+VCAR_REF_NO_temp.elementAt(i)+"' AND CONTRACT_TYPE='O' "
							+ "AND FGSA_NO='"+fgsaNo+"' AND FGSA_REV_NO='"+fgsaRevNo+"' AND SN_NO='"+snNo+"' AND SN_REV_NO='"+snRevNo+"' AND CUSTOMER_CD='"+customerCd+"' "
							+ "ORDER BY MODIFICATION_SEQ_NO DESC";// TO_CHAR(NEW_PRICE_EFF_DT,'DD/MM/YYYY'),FLAG, NEW_SALE_PRICE ASC";
				//	System.out.println("STEP-6: FMS8_CARGO_ALLOC_REVISED_DTL: "+queryString);
					...
					while(rset4.next()) {
						...
						//	VCAR_PRICE_TO.add(rset.getString(8)==null?"":rset.getString(8));
							VCAR_CONF_PRICE.add(VCAR_CONF_PRICE_temp.elementAt(i));
							VCONT_FGSA_NO.add("FGSA-"+rset3.getString(1));	
							VCONT_FGSA_REV_NO.add(rset4.getString(2)==null?"":rset4.getString(2));		
							VCONT_SN_NO.add("SN-"+rset4.getString(3));
							VCONT_START_DT.add("");					
							VCONT_END_DT.add("");
							
							VCAR_ALLOC_QTY.add(rset4.getString(5)==null?"":rset4.getString(5));
							VCONT_SALE_PRICE.add(SNLOASalePrice);
							VCONT_COST_PRICE.add(nf2.format(Double.parseDouble(rset4.getString(7))));
							VCONT_MARGIN.add(nf2.format(Double.parseDouble(rset4.getString(8))));//rset.getString(11)==null?"":rset.getString(11));
							VCONT_AVG_MARGIN.add(rset4.getString(9)==null?"":rset4.getString(9));
							VCONT_TOT_MARGIN.add(rset4.getString(10)==null?"":rset4.getString(10));
							VCONT_CUST_CD.add(rset4.getString(11)==null?"0":rset4.getString(11));
							VPRICE_REQ_FLAG.add(""); //SB20200327
							VPRICE_APRV_FLAG.add(rset4.getString(12)==null?"R":rset4.getString(12)); //SB20200327
							
							customer_cd=rset4.getString(11)==null?"0":rset4.getString(11);
							VContType.add("O"); //SB20200327
					//		VMaxNomDt.add("N.A."); //SB20200326
							VNextNewPriceDt.add(rset4.getString(13)==null?"0":rset4.getString(13)); //SB20200326
							VCONT_NEW_SALE_PRICE.add(nf2.format(Double.parseDouble(rset4.getString(14)))); //SB20200326
							VCONT_NEW_MARGIN.add(nf2.format(Double.parseDouble(rset4.getString(15)))); //SB20200326
							VPrice_Change_Dtl.add("");
							VCAR_SNLOA_CNT.add(""); 
					}
					////////////////////////////////////////////////////////////////////
			//		System.out.println("VCONT_COST_PRICE: "+VCONT_COST_PRICE.size());
				//	System.out.println("VCONT_COST_PRICE: "+VCONT_COST_PRICE);				
					}
					//////////////////////For LOA /////////////////////////////////////////////
					queryString = "SELECT B.TENDER_NO,B.TENDER_NO,B.LOA_NO,B.LOA_REV_NO,TO_CHAR(SIGNING_DT,'DD/MM/YYYY'),"
							+ "TO_CHAR(START_DT,'DD/MM/YYYY'),TO_CHAR(END_DT,'DD/MM/YYYY'),ALLOC_QTY, SALE_PRICE, COST_PRICE, MARGIN, AVG_MARGIN,TOTAL_MARGIN,B.CUSTOMER_CD "
							+ ", AUTH_BY, APRV_BY, B.STATUS"+
							" FROM FMS7_LOA_CARGO_DTL B, FMS7_LOA_MST C WHERE B.CARGO_REF_NO='"+VCAR_REF_NO_temp.elementAt(i)+"' "//DELV_TO_DT >=TO_DATE('"+SearchDt+"','DD/MM/YYYY') "
		...
		//		System.out.println("STEP-1:LOA "+queryString);
				...
				while(rset2.next()) {
					...
					VCAR_CONF_ENG_UNIT.add(VCAR_CONF_ENG_UNIT_temp.elementAt(i));			
					VCAR_CONF_PRICE.add(VCAR_CONF_PRICE_temp.elementAt(i));
					VCONT_FGSA_NO.add("TENDER-"+rset2.getString(1));	
					VCONT_FGSA_REV_NO.add(rset2.getString(2)==null?"":rset2.getString(2));		
					VCONT_SN_NO.add("LoA-"+rset2.getString(3));
					VCONT_START_DT.add(rset2.getString(6)==null?"":rset2.getString(6));					
					VCONT_END_DT.add(rset2.getString(7)==null?"":rset2.getString(7));
					
					VCAR_ALLOC_QTY.add(rset2.getString(8)==null?"":rset2.getString(8));
					VCONT_SALE_PRICE.add(nf2.format(Double.parseDouble(rset2.getString(9))));
					VCONT_COST_PRICE.add(nf2.format(Double.parseDouble(rset2.getString(10))));
				//	VCONT_MARGIN.add(nf2.format(Double.parseDouble(rset2.getString(11))));//rset.getString(11)==null?"":rset.getString(11));
					VCONT_MARGIN.add(nf2.format(Double.parseDouble(rset2.getString(11)==null?"0":rset2.getString(11))));
					VCONT_AVG_MARGIN.add(rset2.getString(12)==null?"":rset2.getString(12));
					VCONT_TOT_MARGIN.add(rset2.getString(13)==null?"":rset2.getString(13));
					VCONT_CUST_CD.add(rset2.getString(14)==null?"0":rset2.getString(14));
					VPRICE_REQ_FLAG.add(rset2.getString(15)==null?"":rset2.getString(15)); //SB20200327
					VPRICE_APRV_FLAG.add(rset2.getString(17)==null?"":rset2.getString(17)); //SB20200327
					VContType.add("L"); //SB20200327
					customer_cd=rset2.getString(14)==null?"0":rset2.getString(14);
				//	VMaxNomDt.add(MaxNomDt==null?"N.A.":MaxNomDt); //SB20200326
					VNextNewPriceDt.add(MaxNewPriceDt); //SB20200326
					VCONT_NEW_SALE_PRICE.add(MaxNewSalePrice); //SB20200326
					VCONT_NEW_MARGIN.add(""); //SB20200326
				...
				//}
					//////////////////////////LOA -Cargo Price Dtl///////////////////////////////////////
					String fgsaNo=""+rset2.getString(1); String fgsaRevNo=""+rset2.getString(2); String snNo=""+rset2.getString(3); String snRevNo=""+rset2.getString(4); String customerCd=""+customer_cd;
			queryString = "SELECT FGSA_NO,FGSA_REV_NO,SN_NO,SN_REV_NO,ALLOC_QTY, NEW_SALE_PRICE, ORI_SALE_PRICE, ORI_MARGIN,  ORI_AVG_MARGIN,ORI_TOT_MARGIN, CUSTOMER_CD, FLAG, TO_CHAR(NEW_PRICE_EFF_DT,'DD/MM/YYYY'),NEW_SALE_PRICE, NEw_MARGIN "
					+ "FROM FMS8_CARGO_ALLOC_REVISED_DTL WHERE CARGO_REF_NO='"+VCAR_REF_NO_temp.elementAt(i)+"' AND CONTRACT_TYPE='G' "
					+ "AND FGSA_NO='"+fgsaNo+"' AND FGSA_REV_NO='"+fgsaRevNo+"' AND SN_NO='"+snNo+"' AND SN_REV_NO='"+snRevNo+"' AND CUSTOMER_CD='"+customerCd+"' "
					+ "ORDER BY MODIFICATION_SEQ_NO DESC";// TO_CHAR(NEW_PRICE_EFF_DT,'DD/MM/YYYY'),FLAG, NEW_SALE_PRICE ASC";
	//		System.out.println("STEP-6: FMS8_CARGO_ALLOC_REVISED_DTL: "+queryString);
			...
			while(rset5.next()) {
				...
				//	VCAR_PRICE_TO.add(rset.getString(8)==null?"":rset.getString(8));
					VCAR_CONF_PRICE.add(VCAR_CONF_PRICE_temp.elementAt(i));
					VCONT_FGSA_NO.add("TENDER-"+rset2.getString(1));	
					VCONT_FGSA_REV_NO.add(rset5.getString(2)==null?"":rset5.getString(2));		
					VCONT_SN_NO.add("LoA-"+rset5.getString(3));
					VCONT_START_DT.add("");					
					VCONT_END_DT.add("");
					
					VCAR_ALLOC_QTY.add(rset5.getString(5)==null?"":rset5.getString(5));
					VCONT_SALE_PRICE.add(SNLOASalePrice);
					VCONT_COST_PRICE.add(nf2.format(Double.parseDouble(rset5.getString(7))));
					VCONT_MARGIN.add(nf2.format(Double.parseDouble(rset5.getString(8))));//rset.getString(11)==null?"":rset.getString(11));
					VCONT_AVG_MARGIN.add(rset5.getString(9)==null?"":rset5.getString(9));
					VCONT_TOT_MARGIN.add(rset5.getString(10)==null?"":rset5.getString(10));
					VCONT_CUST_CD.add(rset5.getString(11)==null?"0":rset5.getString(11));
					VPRICE_REQ_FLAG.add(""); //SB20200327
					VPRICE_APRV_FLAG.add(rset5.getString(12)==null?"R":rset5.getString(12)); //SB20200327
					
					customer_cd=rset5.getString(11)==null?"0":rset5.getString(11);
					VContType.add("G"); //SB20200327
			//		VMaxNomDt.add("N.A."); //SB20200326
					VNextNewPriceDt.add(rset5.getString(13)==null?"0":rset5.getString(13)); //SB20200326
					VCONT_NEW_SALE_PRICE.add(nf2.format(Double.parseDouble(rset5.getString(14)))); //SB20200326
					VCONT_NEW_MARGIN.add(nf2.format(Double.parseDouble(rset5.getString(15)))); //SB20200326
					VPrice_Change_Dtl.add("");
					VCAR_SNLOA_CNT.add(""); 
					}
				}
				VCAR_SNLOA_CNT.add(""+SN_LOA_Count);  ///SN_LOA_Count==0 i.e., No SN /LoA
				}
			////////////////////////////////////////////////////////////////////
		//	System.out.println("VCAR_SNLOA_CNT: "+VCAR_SNLOA_CNT.size()); System.out.println("****: "+VCAR_SNLOA_CNT);
			////////////////////////////////////////////////////////////////////////////	
	/////////Get Past Price Change Record/////////////////////SB20200330////////////////////////////////
						System.out.println("CARGO-SIZE VCAR_REF_NO  :"+VCAR_REF_NO.size());
						for(int i=0; i<VCAR_REF_NO.size(); i++)
						{
						String start_dt =""; double ori_sale_price=0;  double ori_margin=0; double new_sale_price=0; double new_margin=0;
						start_dt = ""+VCONT_START_DT.elementAt(i);
					//	System.out.println("VContType: "+VContType.elementAt(i));System.out.println("****: "+VCAR_SNLOA_CNT.elementAt(i));
						queryString1 = "SELECT CUSTOMER_CD,CUSTOMER_NAME,CUSTOMER_ABBR FROM FMS7_CUSTOMER_MST "
								+ "where (DORMANT_FLAG!='Y' OR DORMANT_FLAG IS NULL) AND CUSTOMER_CD='"+VCONT_CUST_CD.elementAt(i)+"'"
								+ "ORDER BY CUSTOMER_CD";
					//	System.out.println("FMS7_CUSTOMER_MST Fetch Query = "+queryString1);
						rset1 = stmt.executeQuery(queryString1);				
						if(rset1.next())
						{
							if(VContType.elementAt(i).equals("O") || VContType.elementAt(i).equals("G"))
								VCONT_CUST_NM.add("");
							else if(VContType.elementAt(i).equals("X") && VCAR_SNLOA_CNT.elementAt(i).equals(""))
								VCONT_CUST_NM.add("Allocated Qty to Customer(s)");
							else 
								VCONT_CUST_NM.add(rset1.getString(2)+" ("+rset1.getString(3)+")");
						}	
						else
						{
							if(VContType.elementAt(i).equals("X") && VCAR_SNLOA_CNT.elementAt(i).equals("0"))
								VCONT_CUST_NM.add("Yet to Allocate Qty to Customer");
							else
								VCONT_CUST_NM.add("");
						}
						///////////////SB20200525: Custom Duty Paid Final Status/////////////////////////
						double ConfPriceSellerFinal=0; double ConfPriceCDFinal=0; double ConfPrice=0;
						queryString1 = "SELECT CONFIRM_PRICE, TAX_STR_CD, APPROVED_FLAG FROM FMS7_FINAL_CUSTOM_DUTY "
								+ "where APPROVED_FLAG='Y' AND CARGO_REF_NO='"+VCAR_REF_NO.elementAt(i)+"' AND FLAG='Y'";
					//	System.out.println("FMS7_CUSTOMER_MST Fetch Query = "+queryString1);
						rset1 = stmt.executeQuery(queryString1);				
						if(rset1.next())
						{
						//	VCAR_CONFIRM_FINAL_PRICE.add(nf2.format(Double.parseDouble(rset1.getString(1)==null?"0":rset1.getString(1))));
							ConfPriceCDFinal=rset1.getDouble(1); System.out.println("CUST-DUTY: ConfPrice: "+ConfPrice);
							ConfPrice=ConfPriceCDFinal;
						}	
						
						queryString1 = "SELECT CONFIRM_PRICE, APPROVED_FLAG FROM FMS7_FINAL_SELLER_PAY "
								+ "where APPROVED_FLAG='Y' AND CARGO_REF_NO='"+VCAR_REF_NO.elementAt(i)+"' AND FLAG='Y'";
					//	System.out.println("FMS7_CUSTOMER_MST Fetch Query = "+queryString1);
						rset1 = stmt.executeQuery(queryString1);				
						if(rset1.next())
						{
							//VCAR_CONFIRM_FINAL_PRICE.add(nf2.format(Double.parseDouble(rset1.getString(1)==null?"0":rset1.getString(1))));
							ConfPriceSellerFinal=rset1.getDouble(1); System.out.println("SELLER_FINAL: ConfPrice: "+ConfPrice);
							ConfPrice=ConfPriceSellerFinal;
						}	
						
						if(ConfPriceCDFinal>0 && ConfPriceSellerFinal==0)
							VCAR_CONFIRM_FINAL_PRICE.add("CD(Final) Approved for "+ConfPriceCDFinal);
						else if(ConfPriceCDFinal==0 && ConfPriceSellerFinal>0)
							VCAR_CONFIRM_FINAL_PRICE.add("Seller Payment(Final) Approved for "+ConfPriceSellerFinal);
						else if(ConfPrice==0)
							VCAR_CONFIRM_FINAL_PRICE.add(""); 
						else
							VCAR_CONFIRM_FINAL_PRICE.add("Seller Payment(Final) and CD(Final) Approved for "+ConfPrice);
						/////////////////////////////////////////////////////////////////////////////

						}
				//	System.out.println("****: "+VCONT_COST_PRICE.size() ); System.out.println("****: "+VCONT_COST_PRICE ); 
				//		System.out.println("****: "+VPRICE_APRV_FLAG.size() ); System.out.println("VPRICE_APRV_FLAG***: "+VPRICE_APRV_FLAG );
				//		System.out.println("****: "+VOriMargin.size() ); System.out.println("****: "+VNewMargin.size() ); 
				//		System.out.println("****: "+VPrice_Change_Dtl.size() );System.out.println("****: "+VPrice_Change_Dtl );//System.out.println("****: "+VMaxNomDt.size() );
						////////////////////////////////////////////////////////////////////////
		} catch(Exception e) {
			e.printStackTrace();
		}
	}
	public void auditRpt_price_change()
	{
		try
		{	
			rate_flag="%";
			queryString = "SELECT FGSA_NO,FGSA_REV_NO,SN_NO,SN_REV_NO,CONTRACT_TYPE,CUSTOMER_CD, "
					+ "ORI_SALE_PRICE, NEW_SALE_PRICE, ORI_MARGIN, NEW_MARGIN, ORI_AVG_MARGIN, NEW_AVG_MARGIN, ORI_TOT_MARGIN, NEW_TOT_MARGIN, "
					+ "CARGO_REF_NO, ALLOC_QTY, TO_CHAR(NEW_PRICE_EFF_DT,'DD/MM/YYYY'), "
					+ "ENT_BY, TO_CHAR(ENT_DT,'DD/MM/YYYY HH24:MI:SS'), APPROVE_BY, TO_CHAR(APPROVE_DT,'DD/MM/YYYY HH24:MI:SS'), MODIFICATION_SEQ_NO, FLAG, REMARK "
					+ "FROM FMS8_CARGO_ALLOC_REVISED_DTL WHERE CARGO_REF_NO like '"+rate_flag+"' AND CONTRACT_TYPE LIKE '"+index_type+"' "
					+ "ORDER BY CARGO_REF_NO, MODIFICATION_SEQ_NO DESC";// TO_CHAR(NEW_PRICE_EFF_DT,'DD/MM/YYYY'),FLAG, NEW_SALE_PRICE ASC";
			System.out.println("STEP-6: FMS8_CARGO_ALLOC_REVISED_DTL: "+queryString);
			rset5 = stmt5.executeQuery(queryString);
			while(rset5.next()) {
					VCONT_FGSA_NO.add("TENDER-"+rset5.getString(1));	
					VCONT_FGSA_REV_NO.add(rset5.getString(2)==null?"":rset5.getString(2));		
					VCONT_SN_NO.add("LoA-"+rset5.getString(3));
					VCONT_SN_REV_NO.add("LoA-"+rset5.getString(4));
					VContType.add(rset5.getString(5)); //SB20200327
					VCONT_CUST_CD.add(rset5.getString(6)==null?"0":rset5.getString(6));
					VCONT_SALE_PRICE.add(rset5.getString(7)==null?"0":rset5.getString(7));
					VCONT_NEW_SALE_PRICE.add(nf2.format(Double.parseDouble(rset5.getString(8))));
					VCONT_MARGIN.add(nf2.format(Double.parseDouble(rset5.getString(9))));//rset.getString(11)==null?"":rset.getString(11));
					VCONT_NEW_MARGIN.add(nf2.format(Double.parseDouble(rset5.getString(10))));
					VCONT_AVG_MARGIN.add(rset5.getString(11)==null?"":rset5.getString(11));
					VCONT_NEW_AVG_MARGIN.add(rset5.getString(12)==null?"":rset5.getString(12));
					VCONT_TOT_MARGIN.add(rset5.getString(13)==null?"":rset5.getString(13));
					VCONT_NEW_TOT_MARGIN.add(rset5.getString(14)==null?"":rset5.getString(14));
					VCAR_REF_NO.add(rset5.getString(15)==null?"":rset5.getString(15));
					VCAR_ALLOC_QTY.add(rset5.getString(16)==null?"":rset5.getString(16));
					VNextNewPriceDt.add(rset5.getString(17)==null?"0":rset5.getString(17)); //SB20200326
						
					customer_cd=rset5.getString(11)==null?"0":rset5.getString(11);
					VCAR_PriceEntBy.add(rset5.getString(18)==null?"0":rset5.getString(18));
					VCAR_PriceEntDt.add(rset5.getString(19)==null?"":rset5.getString(19));
					VCAR_PriceAprvBy.add(rset5.getString(20)==null?"0":rset5.getString(20));
					VCAR_PriceAprvDt.add(rset5.getString(21)==null?"":rset5.getString(21));
					VPRICE_APRV_FLAG.add(rset5.getString(23)==null?"":rset5.getString(23)); 
					VCAR_PRICE_REMARK.add(rset5.getString(24)==null?"":rset5.getString(24)); 
					}
for(int i=0; i<VCAR_REF_NO.size(); i++)
{
			queryString="select EMP_NM from HR_EMP_MST WHERE EMP_CD='"+VCAR_PriceEntBy.elementAt(i)+"' ";
			rset=stmt.executeQuery(queryString);
			if(rset.next())
				VCAR_PriceEntNm.add(rset.getString("EMP_NM"));
			else
				VCAR_PriceEntNm.add("");
			queryString="select EMP_NM from HR_EMP_MST WHERE EMP_CD='"+VCAR_PriceAprvBy.elementAt(i)+"' ";
			rset=stmt.executeQuery(queryString);
			if(rset.next())
				VCAR_PriceAprvNm.add(rset.getString("EMP_NM"));
			else
				VCAR_PriceAprvNm.add("");
}
		}
		catch(Exception e)
		{
//			System.out.println("EXCEPTION:DataBean_Master --> get_interest_exchg_Rate() --> "+e.getMessage());
			e.printStackTrace();
		}
	}
	public void get_community_Rate()
	{
		Vector temp_Vcurren_cd = new Vector();
		Vector temp_Vcurren_cd_from = new Vector();
		try
		{
			String start_dt =""; String delv_mth ="01/"+month+"/"+year; int businessMonth=0; int businessYear=0; String StartMonth="";String EndMonth="";
			if(Integer.parseInt(month)>2 && Integer.parseInt(month)<=12)
			{
				businessMonth=(Integer.parseInt(month)-2);
				businessYear=(Integer.parseInt(year));
			}
			else
			{
				if(Integer.parseInt(month)==2) {businessMonth=12;}
				if(Integer.parseInt(month)==1) {businessMonth=11;}
			...
			if(businessMonth<=9)
			{
				...
				start_dt = "16/"+"0"+businessMonth+"/"+year;
			}
			else
			{
				StartMonth=""+businessMonth;
				start_dt = "16/"+businessMonth+"/"+year;
			}
		...
		//	System.out.println("start_dt = "+start_dt);
			...
			//	System.out.println("Query for generating Last Date of the selected Month = "+queryString);
				...
				if(businessMonth<=9)
				{
					start_dt = "01/"+"0"+businessMonth+"/"+year;
					EndMonth="0"+businessMonth;
				}
				else
				{
					start_dt = "01/"+businessMonth+"/"+year;
			...
			//	System.out.println("Query for generating Last Date of the selected Month = "+queryString);
				...
				for(int i=1; i<=15; i++)
				{
					if(i<=9)
					{
						date_arr = "0"+i+"/"+EndMonth+"/"+year;
					}
					else
					{
						date_arr = i+"/"+EndMonth+"/"+year;
					}					
					Veff_dt.add(date_arr);
				}
			
			
			double sumCommodityValue=0; int countNonZero=0; double avgCommodityValue=0; 
			if(rate_flag.equalsIgnoreCase("Commodity"))
			{
				for(int i=0; i<Veff_dt.size(); i++)
				{
					queryString = "select COMMODITY_VALUE, REMARK from FMS9_COMMODITY_VALUE_DTL "
							+ "where DELV_MTH=to_date('"+delv_mth+"','dd/mm/yyyy') and BUSINESS_DT=to_date('"+Veff_dt.elementAt(i)+"','dd/mm/yyyy') AND INDEX_TYPE='"+index_type+"' AND FLAG='Y'";
			//		System.out.println("COMMODITY: "+queryString);
					rset = stmt.executeQuery(queryString);
					if(rset.next())
					{
						rate_value = rset.getString(1)==null?"0":rset.getString(1);
						remarks = rset.getString(2)==null?"":rset.getString(2);
						
						rate_value = nf2.format(Double.parseDouble(rate_value));
						
						Vrate_value.add(rate_value);
						Vremark.add(remarks);
						sumCommodityValue=sumCommodityValue+Double.parseDouble(rate_value);
						countNonZero++;
						
					}
					else
					{
						rate_value = nf.format(Double.parseDouble("0"));
						remarks = "";
			...
			avgCommodityValue=sumCommodityValue/countNonZero;
			AvgCommodityValue=nf2.format(avgCommodityValue);
			String factor="0.36";
			ContPriceBasedCommodityValue=nf2.format(avgCommodityValue-Double.parseDouble(""+factor));
		}
		catch(Exception e)
		{
			System.out.println("EXCEPTION:DataBean_Master --> get_Inserted_Interest_Exchg_Rate_Value() --> "+e.getMessage());
			e.printStackTrace();
		}
	}
	public void CheckForColumn_FMS7_MAN_CONFIRM_CARGO_DTL()
	{
		try
		{
			int count=0;
			String query="SELECT COUNT(COLUMN_NAME) FROM COLS WHERE UPPER(TABLE_NAME) LIKE 'FMS7_MAN_CONFIRM_CARGO_DTL' "
					+ "AND UPPER(COLUMN_NAME) LIKE 'FINAL_PRICE_DT' ";
			rset=stmt.executeQuery(query);
			if(rset.next())
			{
				count=rset.getInt(1);
			...
			if(count==0)
			{
				query="alter table FMS7_MAN_CONFIRM_CARGO_DTL add FINAL_PRICE_DT date";
				stmt.executeUpdate(query);
				query="alter table FMS7_MAN_CONFIRM_CARGO_DTL add FINAL_PRICE_APRV char(1)";
				stmt.executeUpdate(query);
				conn.commit();
			}
		}
...
////Following (4) String Variables Has Been Defined By Jaimin Patel On 23rd Nov, 2011 ...
	...
	public void setRate_flag(String rate_flag) {this.rate_flag = rate_flag;}
	String int_rate_flag=""; 
	String exg_rate_flag="";
	String eff_date = "";
	String rate_flag ="";
	...
	String rate_value = "";
	String curren_cd = "";
	String curren_cd_from = "";
	String AvgCommodityValue = "";
	public String getAvgCommodityValue() {return AvgCommodityValue;}
	String ContPriceBasedCommodityValue = "";
	public String getContPriceBasedCommodityValue() {return ContPriceBasedCommodityValue;}
	public Vector int_rate_cd=new Vector();
	public Vector int_rate_nm=new Vector();
	public Vector exg_rate_nm=new Vector();
	public Vector exg_rate_cd=new Vector();
	public Vector currency_cd=new Vector();
	public Vector currency_nm=new Vector();
	public Vector Vrate_value = new Vector();
	public Vector Vcurren_cd = new Vector();
	public Vector Vcurren_cd_from = new Vector();
	public Vector Vremark = new Vector();
	public Vector Veff_dt = new Vector();
	public Vector Vcomponent_cd = new Vector();
	public Vector Vcomponent_nm = new Vector();
	public Vector Vunit_cd = new Vector();
	public Vector Vunit_nm = new Vector();
	public Vector getInt_rate_cd() {return int_rate_cd;}
	public Vector getInt_rate_nm() {return int_rate_nm;}
	public Vector getExg_rate_nm() {return exg_rate_nm;}
	public Vector getExg_rate_cd() {return exg_rate_cd;}
	public Vector getVrate_value() {return Vrate_value;}
	public Vector getVcurren_cd() {return Vcurren_cd;}
	public Vector getCurrency_nm() {return currency_nm;}
	public Vector getCurrency_cd() {return currency_cd;}
	public Vector getVcurren_cd_from() {return Vcurren_cd_from;}
	public Vector getVremark() {return Vremark;}
	public Vector getVeff_dt() {return Veff_dt;}
	public Vector getVcomponent_cd() {return Vcomponent_cd;}
	public Vector getVcomponent_nm() {return Vcomponent_nm;}
	Vector int_rate_bank_name = new Vector();
	public Vector getInt_rate_bank_name() {return int_rate_bank_name;}
	//Following String Getter Methods are defined by Samik Shah ... on 4th March, 2010 ...
	public String getExg_rate_flag() {return exg_rate_flag;}
	public String getInt_rate_flag() {return int_rate_flag;}
	public String getCurren_cd() {return curren_cd;}
	public String getRate_value() {return rate_value;}
	public String getRemarks() {return remarks;}
	public String getCurren_cd_from() {return curren_cd_from;}

	//Following Vectors Getter Methods are defined by Samik Shah ... on 5th March, 2010 ...
	public Vector getVunit_cd() {return Vunit_cd;}
	public Vector getVunit_nm() {return Vunit_nm;}
	//////////////////////////////////////////////////////////////
	...
	Vector VOriPrice = new Vector();
	public Vector getVOriPrice() {return VOriPrice;}
	Vector VOriMargin = new Vector();
	public Vector getVOriMargin() {return VOriMargin;}
	Vector VNewMargin = new Vector();
	public Vector getVNewMargin() {return VNewMargin;}
	...
	//////////////////CARGO PRICE CHANGE/////////////////////////////
	Vector VCAR_PRICE_TYPE= new Vector();
	public Vector getVCAR_PRICE_TYPE() {return VCAR_PRICE_TYPE;} 
	Vector VCAR_SNLOA_CNT= new Vector();
	public Vector getVCAR_SNLOA_CNT() {return VCAR_SNLOA_CNT;} 
	Vector VCAR_PRICE_REMARK= new Vector();
	public Vector getVCAR_PRICE_REMARK() {return VCAR_PRICE_REMARK;} 
	Vector VCAR_CUST_DUTY_PAID= new Vector();
	public Vector getVCAR_CUST_DUTY_PAID() {return VCAR_CUST_DUTY_PAID;} 
	Vector VCAR_CONFIRM_FINAL_PRICE= new Vector();
	public Vector getVCAR_CONFIRM_FINAL_PRICE() {return VCAR_CONFIRM_FINAL_PRICE;} //SB20200511
	Vector VCAR_CONF_SEQ= new Vector();
	public Vector getVCAR_CONF_SEQ() {return VCAR_CONF_SEQ;} 
	Vector VCAR_REF_NO= new Vector();
	public Vector getVCAR_REF_NO() {return VCAR_REF_NO;} //SB20200511//SB20200511
	Vector VCAR_DELV_FM= new Vector();
	public Vector getVCAR_DELV_FM() {return VCAR_DELV_FM;} //SB20200511
	Vector VCAR_DELV_TO= new Vector();
	public Vector getVCAR_DELV_TO() {return VCAR_DELV_TO;} //SB20200511
	Vector VCAR_CONF_ENG= new Vector();
	public Vector getVCAR_CONF_ENG() {return VCAR_CONF_ENG;} //SB20200511
	Vector VCAR_CONF_ENG_UNIT= new Vector();
	public Vector getVCAR_CONF_ENG_UNIT() {return VCAR_CONF_ENG_UNIT;} //SB20200511
	Vector VCAR_PRICE_FM= new Vector();
	public Vector getVCAR_PRICE_FM() {return VCAR_PRICE_FM;} //SB20200511
	Vector VCAR_PRICE_TO= new Vector();
	public Vector getVCAR_PRICE_TO() {return VCAR_PRICE_TO;} //SB20200511
	Vector VCAR_CONF_PRICE= new Vector();
	public Vector getVCAR_CONF_PRICE() {return VCAR_CONF_PRICE;} //SB20200511
	Vector VCONT_FGSA_NO= new Vector();
	public Vector getVCONT_FGSA_NO() {return VCONT_FGSA_NO;} //SB20200511
	Vector VCONT_FGSA_REV_NO= new Vector();
	public Vector getVCONT_FGSA_REV_NO() {return VCONT_FGSA_REV_NO;} //SB20200511
	Vector VCONT_SN_NO= new Vector();
	public Vector getVCONT_SN_NO() {return VCONT_SN_NO;} //SB20200511
	Vector VCONT_SN_REV_NO= new Vector();
	public Vector getVCONT_SN_REV_NO() {return VCONT_SN_REV_NO;} //SB20200511
	Vector VCONT_START_DT= new Vector();
	public Vector getVCONT_START_DT() {return VCONT_START_DT;} //SB20200511
	Vector VCONT_END_DT= new Vector();
	public Vector getVCONT_END_DT() {return VCONT_END_DT;} //SB20200511
	Vector VCONT_COST_PRICE= new Vector();
	public Vector getVCONT_COST_PRICE() {return VCONT_COST_PRICE;} //SB20200511
	Vector VCONT_SALE_PRICE= new Vector();
	public Vector getVCONT_SALE_PRICE() {return VCONT_SALE_PRICE;} //SB20200511
	Vector VCONT_MARGIN= new Vector();
	public Vector getVCONT_MARGIN() {return VCONT_MARGIN;} //SB20200511
	Vector VCONT_NEW_SALE_PRICE= new Vector();
	public Vector getVCONT_NEW_SALE_PRICE() {return VCONT_NEW_SALE_PRICE;} //SB20200511
	Vector VCONT_NEW_MARGIN= new Vector();
	public Vector getVCONT_NEW_MARGIN() {return VCONT_NEW_MARGIN;} //SB20200511
	Vector VCONT_AVG_MARGIN= new Vector();
	public Vector getVCONT_AVG_MARGIN() {return VCONT_AVG_MARGIN;} //SB20200511
	Vector VCONT_NEW_AVG_MARGIN= new Vector();
	public Vector getVCONT_NEW_AVG_MARGIN() {return VCONT_NEW_AVG_MARGIN;} //SB20200511 
	Vector VCONT_TOT_MARGIN= new Vector();
	public Vector getVCONT_TOT_MARGIN() {return VCONT_TOT_MARGIN;} //SB20200511
	Vector VCONT_NEW_TOT_MARGIN= new Vector();
	public Vector getVCONT_NEW_TOT_MARGIN() {return VCONT_NEW_TOT_MARGIN;} //SB20200511
	Vector VCAR_ALLOC_QTY= new Vector();
	public Vector getVCAR_ALLOC_QTY() {return VCAR_ALLOC_QTY;} //SB20200511
	Vector VCONT_CUST_CD= new Vector();
	public Vector getVCONT_CUST_CD() {return VCONT_CUST_CD;} //SB20200511
	Vector VCONT_CUST_NM= new Vector();
	public Vector getVCONT_CUST_NM() {return VCONT_CUST_NM;} //SB20200511
	Vector VMaxNewPriceDt=new Vector();
	public Vector getVMaxNewPriceDt() {return VMaxNewPriceDt;} //SB20200511
	Vector VContType= new Vector();
	public Vector getVContType() {return VContType;} //SB20200511
	Vector VCAR_PriceEntBy= new Vector();
	public Vector getVCAR_PriceEntBy() {return VCAR_PriceEntBy;} //SB20200511
	Vector VCAR_PriceEntNm= new Vector();
	public Vector getVCAR_PriceEntNm() {return VCAR_PriceEntNm;} //SB20200511
	Vector VCAR_PriceEntDt= new Vector();
	public Vector getVCAR_PriceEntDt() {return VCAR_PriceEntDt;} //SB20200511
	Vector VCAR_PriceAprvBy= new Vector();
	public Vector getVCAR_PriceAprvBy() {return VCAR_PriceAprvBy;} //SB20200511
	Vector VCAR_PriceAprvNm= new Vector();
	public Vector getVCAR_PriceAprvNm() {return VCAR_PriceAprvNm;} //SB20200511
	Vector VCAR_PriceAprvDt= new Vector();
	public Vector getVCAR_PriceAprvDt() {return VCAR_PriceAprvDt;} //SB20200511
	/////////////^20200326///////////////////
