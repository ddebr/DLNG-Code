...
else
{
				  		  ...
				  		  request21.onreadystatechange = function() 
				    	  {
				   			 if(request21.readyState==4)
				     		 {
					 				var a=document.getElementById("salepricediv").innerHTML=request21.responseText; 
					 			  	document.forms[0].double_final_gross_amt_inr.value=round(document.forms[0].grosssalespriceINR.value,0);
	...
	else
	{
						...
						{
							document.forms[0].double_final_gross_amt_inr.value=round(document.forms[0].gross_amt_inr.value,0);
							grossamt=document.forms[0].double_final_gross_amt_inr.value;
						}
						var taxcd=document.forms[0].taxstructcd.value;
						caluclate_tax_details_ajax(grossamt,taxcd);
	...
	if(flag==true)
	{
		...
		else
		{
			...
			if(cur==0)
			{
				...
				document.forms[0].priceINR2.value=round(calculatedamtinr,0);
			}
			else if(cur==1)
			{
...
function adjustmamountforadjust()
{
	...
	if(flag1==true)
	{
		...
		if(flag==true)
		{
			...
			else
			{
				...
				{	
					...
					var calculatedamt=0;
					var calculatedamtinrnew=0;
					if(adjustsign=='1')
					{
						if(parseFloat(grossusd)==parseFloat(adjustamt)) {
							calculatedamt=0;
							var adjustamtinr=grossinr;
							calculatedamtinrnew=0;
						} else {
							calculatedamt=parseFloat(grossusd)-parseFloat(adjustamt);
							var adjustamtinr=parseFloat(adjustamt)*parseFloat(exchrate);
							calculatedamtinrnew=parseFloat(grossinr)-parseFloat(adjustamtinr);
						}
					}
					else if(adjustsign=='2')
					{
					...
					else
					{
						...
						document.forms[0].inrAmt1.value=round(calculatedamtinrnew,0); 
						
							if(document.forms[0].adjustdetails[1].checked){ 
								document.forms[0].double_final_gross_amt_inr.value=round(calculatedamtinrnew,0);
								...
								for(l=0;l<taxratesize;l++){
									...
									document.getElementById("tax_amts"+l).value=round((round(calculatedamtinrnew,2)*taxrate/100),0);
									taxadjamt+=parseFloat(document.getElementById("tax_amts"+l).value);
								}
								document.forms[0].total_tax.value=round(taxadjamt,0);
								document.forms[0].net_amt_inr.value=round((parseFloat(calculatedamtinrnew)+parseFloat(taxadjamt)),0);
							}
							var grs_inr="0";
							if(document.forms[0].tax_codes_gst) {
								var tax_gst_size = document.forms[0].tax_codes_gst.length;
								if(tax_gst_size==undefined) {
									tax_gst_size = '1';
								}
								for(var c=0;c<parseFloat(tax_gst_size);c++) {
									var adjustamttax = document.getElementById("adjustamt_tax"+c).value;
									if(adjustamttax=="") {
										grs_inr=document.forms[0].total_tax.value;
										document.getElementById("gross_amt_inr_adjusted_tax"+c).value = round(grs_inr,0);
									} else {
										var amt = adjustamttax;
										if(c==0) {
											document.getElementById("gross_amt_inr_adjusted_tax"+c).value = round((parseFloat(document.forms[0].total_tax.value)-parseFloat(amt)),0);
										} else {
											var previous_amt = document.getElementById("adjustamt_tax"+(c-1)).value;
											if(previous_amt=='') previous_amt = 0;
											document.getElementById("gross_amt_inr_adjusted_tax"+c).value = round((parseFloat(document.forms[0].total_tax.value)-parseFloat(amt)-parseFloat(previous_amt)),0);
										}
										grs_inr=document.getElementById("gross_amt_inr_adjusted_tax"+c).value;
									}
								}
							}
							document.forms[0].net_amt_inr.value=round((parseFloat(grs_inr)+parseFloat(calculatedamtinrnew)),0);
					}
				}
				else if(cur==1)
				{
					...
					else
					{
						document.forms[0].gross_amt_inr_adjusted.value=round(calculatedamt,0);
						document.forms[0].priceINR2.value=round(calculatedamt,0);
						document.forms[0].inrAmt1.value=round(calculatedamt,0); 
						if(document.forms[0].adjustdetails[1].checked){ 
							document.forms[0].double_final_gross_amt_inr.value=round(calculatedamt,0);
							var taxratesize=document.getElementById('taxratesize').value;
							var taxadjamt=0;
							for(l=0;l<taxratesize;l++){
								var taxrate=document.getElementById('taxrate'+l).value;
								document.getElementById("tax_amts"+l).value=round((round(calculatedamt,2)*taxrate/100),0);
								taxadjamt+=parseFloat(document.getElementById("tax_amts"+l).value);
							}
							document.forms[0].total_tax.value=round(taxadjamt,0);
							document.forms[0].net_amt_inr.value=round(parseFloat(round(calculatedamt,0)+round(taxadjamt,0)),0);
						}
						var grs_inr="0";
						if(document.forms[0].tax_codes_gst) {
							var tax_gst_size = document.forms[0].tax_codes_gst.length;
						...
						document.forms[0].net_amt_inr.value=round((parseFloat(grs_inr)+parseFloat(calculatedamt)),0);
					}
				}
		...
		{
			alert("Amount To Be Adjusted Is Greater Than Available Balance..!!");
			document.forms[0].adjustamt.value="";
			document.forms[0].gross_amt_inr_adjusted.value="";
			document.forms[0].priceINR2.value="";
...
function adjustmamountforadjust_tax(index)
{
	var adjustamt=document.getElementById("adjustamt_tax"+index).value;
	var advanceamt=document.getElementById("availablebalance_tax"+index).value;
	var negflag=false;
	var flag1=checkNumber1(document.getElementById("adjustamt_tax"+index),12,2);
	if(flag1==true)
	{
		var flag=checkadjustlimit_tax(advanceamt,adjustamt);
		if(flag==true)
		{
			if(adjustamt=='' || adjustamt==" ")
			{
			...
			{
				var adjustsign=document.getElementById("adjustsign_tax"+index).value;
				var cur='INR';
				if(cur=='INR')
				{
					var grossinr='';
					if(parseFloat(index)==0){
						grossinr = document.forms[0].total_tax.value;
					} else {
						grossinr = document.getElementById("gross_amt_inr_adjusted_tax"+(index-1)).value;
					}
					var calculatedamt=0;
					if(adjustsign=='1')
					{
					...
					{
						document.getElementById("gross_amt_inr_adjusted_tax"+index).value=round(calculatedamt,0);
						var grs_inr="0";
						if(document.forms[0].priceINR2.value==""){}
						else{ grs_inr=document.forms[0].priceINR2.value;}
						
						var len = document.forms[0].tax_codes_gst.length;
						if(len==undefined) {
							len = '1';
						}
						for(var tk=parseFloat(index)+1;tk<parseFloat(len);tk++) {
							var t1_adjustment = document.getElementById("adjustamt_tax"+tk).value;
							if(t1_adjustment!='')
							{
								document.getElementById("gross_amt_inr_adjusted_tax"+tk).value = round((round(calculatedamt,2) - round(t1_adjustment,2)),0);
								calculatedamt = document.getElementById("gross_amt_inr_adjusted_tax"+tk).value;
							} else {
								document.getElementById("gross_amt_inr_adjusted_tax"+tk).value = round(calculatedamt,0);
							}
						}
						document.forms[0].net_amt_inr.value=round((parseFloat(grs_inr)+parseFloat(calculatedamt)),0);
					}
				...
				if(negflag==true)
				{
						alert("Amount To Be Adjusted Is Greater Then The Total Tax..!!");
						document.getElementById("adjustamt_tax"+index).value="";
						document.getElementById("gross_amt_inr_adjusted_tax"+index).value="";
				}
			}			 
		}
		else
		{
			alert("Amount To Be Adjusted Is Greater Then The Advance Available..!!");
			document.getElementById("net_amt_inr").value="";
			document.getElementById("adjustamt_tax"+index).value="";
			document.getElementById("gross_amt_inr_adjusted_tax"+index).value="";
		}
...
function checkadjustlimit_tax(advanceamt,adjustamt)
{
	var flag=true;
...
var discount_flag_apply="N";
   ...
   //end
	...
	if(mode=='Y')
	{
		...
		var tax_code = document.forms[0].adjustamt_tax;
		if(tax_code) {
			if(tax_code.length==undefined) {
				if(document.getElementById("adjustamt_tax0").value=='' || document.getElementById("adjustamt_tax0").value==' ')
				{
					flag=false;
					msg+="Enter Proper Adjustment Amount For Tax Adjustment Row - 1";
				}
				alert("You Are adjusting "+document.getElementById("adjustamt_tax0").value+" INR From Tax "+document.getElementById("tax_abbr_gst0").value+"!!!");
			} else {
				for(var k=0;k<parseFloat(tax_code.length);k++) {
					if(document.getElementById("adjustamt_tax"+k).value=='' || document.getElementById("adjustamt_tax"+k).value==' ')
					{
						flag=false;
						msg+="Enter Proper Adjustment Amount For Tax Adjustment Row - "+(k+1);
					}
					alert("You Are adjusting "+document.getElementById("adjustamt_tax"+k).value+" INR From Tax "+document.getElementById("tax_abbr_gst"+k).value+"!!!");
				}
			}
		...
		///////////RS 20170412
		if(document.forms[0].chk_mul_adv_inv) {
			var chk_mul_adv_inv = document.forms[0].chk_mul_adv_inv.value;
			if(chk_mul_adv_inv == "Y") {
				var adv_inv_len = document.forms[0].adv_inv.length;
				if(adv_inv_len == undefined) {
			...
			if(request22)
		   	{
		  		 ...
		  		 request22.onreadystatechange = function() 
		    	 {
		  		...
		  		if(document.forms[0].adjustdetails[0].checked){ //HS20160903
					...
					document.forms[0].double_final_gross_amt_inr.value=round(actualadvamt,0);
					var taxratesize=document.getElementById('taxratesize').value;
					var taxadjamt=0;
					for(l=0;l<taxratesize;l++){
						var taxrate=document.getElementById('taxrate'+l).value;
						document.getElementById("tax_amts"+l).value=round((actualadvamt*taxrate/100),0);
						taxadjamt+=parseFloat(document.getElementById("tax_amts"+l).value);
					}
					document.forms[0].total_tax.value=round(taxadjamt,0);
					document.forms[0].net_amt_inr.value=""+round(parseFloat(parseFloat(actualadvamt)+round(taxadjamt,2)),0);
					
					var tax_codes_gst_size = document.forms[0].tax_codes_gst;
					 if(tax_codes_gst_size)
					 {
...
function displaytaxadjustmentdetails()
{
	...
	var advadjflg='';
	if(document.forms[0].adjustdetails[1].checked){  
		advadjflg=document.forms[0].adjustdetails[1].value;
	}else{  
		advadjflg=document.forms[0].adjustdetails[0].value;   
	}
	var inv_date=document.forms[0].exch_rate_dt.value;
	
	var tax_codes_gst_size = document.forms[0].tax_codes_gst.length;
	if(tax_codes_gst_size==undefined) {
		tax_codes_gst_size = '1';
	}
	var text0="<table width='100%'>";
		...
		text0+="</tr>";
		for(var i=0;i<parseFloat(tax_codes_gst_size);i++) {
			text0+="<tr class='content1' valign='middle'>";
			text0+="<td><div align='center'><b>"+document.getElementById("tax_abbr_gst"+i).value+"&nbsp;&nbsp;<input type='text' id='availablebalance_tax"+i+"' name='availablebalance_tax' size='10' value='"+document.getElementById("tax_avail_balance"+i).value+"' style='text-align:right' class='mkRdly' > INR<span id=''></span></b></div></td>";
			text0+="<td><div align='center'><b>"+inv_date+"</b></div></td>";
			text0+="<td><div align='center'><b>";
				if(document.forms[0].activity.value=="update"){ 
					text0+="<input type='text' name='adjustamt_tax' id='adjustamt_tax"+i+"' size='10' style='text-align: right;' value='"+document.getElementById("tax_adv_adjustment"+i).value+"'  onchange='adjustmamountforadjust_tax("+i+");'> INR";
				}else{
					if(document.forms[0].Adv_submitted_flag.value=="Y"){ 
						text0+="<input type='text' name='adjustamt_tax' id='adjustamt_tax"+i+"' size='10' style='text-align: right;' value='"+document.getElementById("tax_adv_adjustment"+i).value+"' onchange='adjustmamountforadjust_tax("+i+");'> INR";
					}else{
						text0+="<input type='text' name='adjustamt_tax' id='adjustamt_tax"+i+"' size='10' style='text-align: right;' value='' onchange='adjustmamountforadjust_tax("+i+");'> INR";
					}
				} 
			text0+="<span id=''></span>";
			text0+="</b></div></td>";
			text0+="<td><div align='center'><b>";
			if(document.forms[0].activity.value=="update"){ 
				text0+="<select name='adjustsign_tax' id='adjustsign_tax"+i+"' onchange='adjustmamountforadjust_tax("+i+");'>";
			...
			if(document.forms[0].activity.value=="update"){ 
				text0+="<input type='text' name='gross_amt_inr_adjusted_tax' id='gross_amt_inr_adjusted_tax"+i+"' size='10' value='"+document.getElementById("tax_inv_amt_inr"+i).value+"' style='text-align:right' class='mkRdly'> INR";
			}else{ 
				text0+="<input type='text' name='gross_amt_inr_adjusted_tax' id='gross_amt_inr_adjusted_tax"+i+"' size='10' value='' style='text-align:right' class='mkRdly'> INR";
			}
			text0+="</b></div></td>";
			text0+="</tr>";
		}
...
{
	var obj = document.forms[0].tax_codes_gst;
	if(obj) {
		if(obj.length==undefined) {
			document.getElementById("adjustamt_tax0").disabled=true;
			document.getElementById("adjustamt_tax0").value='';
			document.forms[0].adjustsign_tax.disabled=true;
			document.getElementById("adjustamt_tax0").className="mkrdly";
			document.forms[0].gross_amt_inr_adjusted_tax.value='';
		} else {
			for(var i=0;i<parseFloat(obj.length);i++) {
				document.getElementById("adjustamt_tax"+i).disabled=true;
				document.getElementById("adjustamt_tax"+i).value='';
				document.getElementById("adjustsign_tax"+i).disabled=true;
				document.getElementById("adjustamt_tax"+i).className="mkrdly";
				document.getElementById("gross_amt_inr_adjusted_tax"+i).value='';
			}
		}
	}
...
function saveadvance_tax_data()
{
	...
	var adjustsign='';
	var tax_code='';
	if(document.forms[0].adjustamt_tax) {
		if(document.forms[0].adjustamt_tax.length==undefined) {
			adjustamt = round(parseFloat(document.getElementById("adjustamt_tax0").value),2);
			adjustsign = document.getElementById("adjustsign_tax0").value;
			priceINR2 = round(parseFloat(document.getElementById("gross_amt_inr_adjusted_tax0").value),0);
			tax_code = document.getElementById("tax_codes_gst0").value;
		} else {
			for(var i=0;i<parseFloat(document.forms[0].adjustamt_tax.length);i++) {
				if(i==0) {
					adjustamt = round(parseFloat(document.getElementById("adjustamt_tax"+i).value),2);
					adjustsign = document.getElementById("adjustsign_tax"+i).value;
					priceINR2 = round(parseFloat(document.getElementById("gross_amt_inr_adjusted_tax"+i).value),0);
					tax_code = document.getElementById("tax_codes_gst"+i).value;
				} else {
					adjustamt += '@' + round(parseFloat(document.getElementById("adjustamt_tax"+i).value),2);
					adjustsign += '@' +document.getElementById("adjustsign_tax"+i).value;
					priceINR2 += '@' +round(parseFloat(document.getElementById("gross_amt_inr_adjusted_tax"+i).value),0);
		...
		{
			flag=true;
			msg+="Enter Tax Amount To Be Adjusted \n";
		}
	}
...
function displaydetailsmodify()
{
		...
		var date=document.forms[0].exch_rate_dt.value;
		var date_flag = document.forms[0].date_flag.value;
		var tax_c = document.forms[0].tax_codes_gst;
		var tax_cod_gst='';
		if(tax_c) {
			if(tax_c.length==undefined) {
				tax_cod_gst = document.getElementById("tax_codes_gst0").value;
			} else {
				for(var i=0;i<parseFloat(tax_c.length);i++) {
					if(i==0) {
						tax_cod_gst = document.getElementById("tax_codes_gst"+i).value;
					} else {
						tax_cod_gst += '@'+document.getElementById("tax_codes_gst"+i).value;
					}
				}
		    			  ...
		    			  var taxcd=document.forms[0].taxstructcd.value;
		    			   var net_amt_inr1=document.getElementById("invoice_inr_amt_adjusted").value;
								...
								if(val=='BA') {
									if(document.forms[0].tax_codes_gst) {
										 var tax_codes_gst_size = document.forms[0].tax_codes_gst.length;
										 if(parseFloat(tax_codes_gst_size)>0 || tax_codes_gst_size==undefined)
										 {
											 displaytaxadjustmentdetails(); 
											 adjustmamountforadjust_tax('0');
										 }
									}
								}
...
// 				////////14062017
		...
		var date=document.forms[0].exch_rate_dt.value;
			...
			if(request22)
		   	{
		  		 var url="../advance_payment/frm_invoice_adjustment_modify_ajax.jsp?tax_cod_gst="+tax_cod_gst+"&date_flag="+date_flag+"&mcontract_type="+mcontract_type+"&activity="+'insert'+"&mhlpl_inv_seq_no2="+mhlpl_inv_seq_no2+"&minv_financial_year="+minv_financial_year+"&mcustomer_cd="+mcustomer_cd+"&mcustomer_plant_seq_no="+mcustomer_plant_seq_no+"&mbill_period_end_dt="+mbill_period_end_dt+"&date="+date+"&mexchangerateselected="+mexchangerateselected+"&mfgsa_no="+mfgsa_no+"&msn_no="+msn_no+"&mAdjust_amt_mst="+mAdjust_amt_mst+"&mAdjust_cur_mst="+mAdjust_cur_mst+"&fgsa_rev_no="+mfgsa_rev_no+"&mapping_id="+mapping_id;
		  		 request22.open("POST",url);
		  		 request22.onreadystatechange = function() 
		    	 {
		    		 if(request22.readyState==4)
...
function hidedetailsmodify()
	{
		...
		document.forms[0].double_final_gross_amt_inr.value=round(document.forms[0].double_final_gross_amt_inr.value,0);
			
		var grossamt=document.forms[0].gross_amt_inr.value;
		var taxcd=document.forms[0].taxstructcd.value;
		
		if(document.forms[0].TAX_ADV_ADJUST_FLAG_MST.value=='Y' || document.forms[0].tax_codes_gst){
...
<%	
	...
	if(page_refresh_flag.equals("N") && activity.equals("update"))
	{
		if(date_flag && contract_type.equals("C")) {
			if(!gross_amt_inr2.equals("")) {
				gross_amt_inr2 = ""+java.lang.Math.round(Double.parseDouble(gross_amt_inr2));
			}
		}
	...
	salesInv.setDate_flag(date_flag);
	salesInv.init();
	...
	//GST
	Vector TAX_CODE = salesInv.getTAX_CODE();
	Vector TAX_ABBR = salesInv.getTAX_ABBR();
	Vector TAX_CUR_SERVICE = salesInv.getTAX_CUR_SERVICE();
	Vector TAX_FLAG_SERVICE = salesInv.getTAX_FLAG_SERVICE();
	Vector TAX_AMT_SERVICE = salesInv.getTAX_AMT_SERVICE();
	Map tax_avail_balance = salesInv.getTax_avail_balance();
	Map tax_adv_flag = salesInv.getTax_adv_flag();
	Map tax_adv_adjustment = salesInv.getTax_adv_adjustment();
	Map tax_adv_curr = salesInv.getTax_adv_curr();
	Map tax_inv_amt_inr = salesInv.getTax_inv_amt_inr();
	...
	{
		customer_Inv_Seq_No = customer_inv_seq_no.substring(0,5);
	}
	...
	else if(msg.length()<10 && activity.equals("update"))
	{
		...
		if(!salesInv.getRec_remark().trim().equals(""))
		{
			rec_remark = salesInv.getRec_remark();
		}
	}
...
//		System.out.println("activi--yt-------"+activity);
	...
	//	System.out.println("activi--yt-------"+mapping_id);
		...
		adv.setDate_flag(date_flag); //FOR GST
		adv.setTAX_CODE(TAX_CODE); // FOR GST
		adv.init();
		 ...
		 tax_adv_adjustment = adv.getTax_adv_adjustment();
		 tax_adv_curr = adv.getTax_adv_curr();
		 tax_adv_flag = adv.getTax_adv_flag();
		 tax_avail_balance = adv.getAdjust_total_avail_bal_gst();
		 tax_inv_amt_inr = adv.getTax_inv_amt_inr();
	}
	...
	String contact_Suppl_GSTIN_NO  = salesInv.getContact_Suppl_GSTIN_NO();
	String contact_Suppl_State_Code = salesInv.getContact_Suppl_State_Code();
	String contact_Suppl_Sac_Code = salesInv.getSac_code();
	...
	<tr class="content1" valign="top">
		<td colspan="1">
			...
			<%	}
				...
				{	%>
					<input type="text" name="invoice_date" value="<%=invoice_date%>" size="8" maxlength="11" onblur="validateDate(this);checkWithInvoice(this);">
			<%	}	%>
		...
		<td colspan="1">
			<div align="center">
				<input type="text" name="due_date" value="<%=due_dt%>" size="8" maxlength="11" onblur="validateDate(this);checkWithInvoice(this);">
...
<%} 
	...
	<td colspan="2"><div align="center"><input type="text" style="text-align:right" size="10" name="total_tax" value="<%=Math.round(total_tax) %>" class="mkrdly" readonly="readonly"></div>
	</td>
	</tr>
		<tr >
		<td colspan="6">
...
<!-- xx	 -->
	...
	<% } else{ %>
		...
		<tr class='content1' valign='middle'>
			<td colspan='6'><div align='right' style='padding-right: 33px'>
				<span id='submited_text_sbc'></span>
				<input type='button' name='advance_sbc_submit' value='Save' onclick='saveadvance_sbc_data();'>
			</div></td>
		</tr>
		</table>
		</td>
	...
	<tr valign="top">
		...
		<td class="content1" colspan="5">
			<div align="left">
				<textarea name="rec_remark" cols="80" rows="3"><%=rec_remark%></textarea>
			</div>
		</td>
	</tr>
	<%} %>
...
<table>
	<tr>
		<td>
		<%for(int t=0;t<TAX_CODE.size();t++) { %>
	<input type="hidden" name="tax_abbr_gst" value="<%=TAX_ABBR.elementAt(t)%>" id="tax_abbr_gst<%=t%>">
	<input type="hidden" name="tax_avail_bal_gst" value="" id="tax_avail_bal_gst<%=t%>">
	<input type="hidden" name="tax_codes_gst" value="<%=TAX_CODE.elementAt(t)%>" id="tax_codes_gst<%=t%>">
	<input type="hidden" name="tax_cur_service_gst" value="<%=TAX_CUR_SERVICE.elementAt(t)%>" id="tax_cur_service_gst<%=t%>">
	<input type="hidden" name="tax_amt_service_gst" value="<%=TAX_AMT_SERVICE.elementAt(t)%>" id="tax_amt_service_gst<%=t%>">
	<input type="hidden" name="tax_flag_gst" value="<%=TAX_FLAG_SERVICE.elementAt(t)%>" id="tax_flag_gst<%=t%>">
	<input type="hidden" name="tax_avail_balance" value="<%=tax_avail_balance.get(TAX_CODE.elementAt(t))%>" id="tax_avail_balance<%=t%>">
	<input type="hidden" name="tax_adv_flag" value="<%=tax_adv_flag.get(TAX_CODE.elementAt(t))%>" id="tax_adv_flag<%=t%>">
	<input type="hidden" name="tax_adv_adjustment" value="<%=tax_adv_adjustment.get(TAX_CODE.elementAt(t))%>" id="tax_adv_adjustment<%=t%>">
	<input type="hidden" name="tax_adv_curr" value="<%=tax_adv_curr.get(TAX_CODE.elementAt(t))%>" id="tax_adv_curr<%=t%>">
	<input type="hidden" name="tax_inv_amt_inr" value="<%=tax_inv_amt_inr.get(TAX_CODE.elementAt(t))%>" id="tax_inv_amt_inr<%=t%>">
<% } %>
...
<input type="hidden" name="tax_Structure_Dtl" value="<%=tax_Structure_Dtl%>">
