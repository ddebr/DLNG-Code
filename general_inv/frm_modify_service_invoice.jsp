...
<title> DLNG | Modify Service Invoice </title>
...
function doSubmit () {
	...
	var inv_seq_no = document.getElementById('new_inv_seq_no').value;
	...
	if(msg!=""){
		...
		var a = confirm('Do You Want To Modify Invoice ? ')
		if(a){
...
// 	alert('in')
	var inr_km = document.getElementById('rate_km').value;
	var inr_mmbtu = document.getElementById('rate_mmbtu').value;
	...
	var new_inv_seq_no = document.getElementById('new_inv_seq_no').value;
	var hlpl_inv_seq_no = document.getElementById('hlpl_inv_seq_no').value;
...
// 	alert(rowno)
	...
	if(msg==""){
		var url = "../general_inv/frm_modify_service_invoice.jsp?supp_cd="+supp_cd+"&cust_cd="+cust_cd+"&cust_plant_cd="+cust_plant_cd+"&modCd="+modCd+"&formId="+formId+"&subModUrl="+subModUrl+"&modUrl="+modUrl+
		"&month="+month+"&year="+year+"&bill_cycle="+bill_cycle+"&mapping_id="+mapping_id+"&inr_km="+inr_km+"&inr_mmbtu="+inr_mmbtu+"&inr_km_str="+inr_km_str+"&km_str="+km_str+"&total_km_inr="+total_km_inr+
		"&invDt="+invDt+"&dueDt="+dueDt+"&calcBase="+calcBase+"&tax_structure="+tax_structure+"&period_end_dt="+period_end_dt+
		"&period_start_dt="+period_start_dt+"&inv_seq_no="+hlpl_inv_seq_no+"&new_inv_seq_no="+new_inv_seq_no+"&fin_yr="+fin_yr+"&cont_type="+contract_type+"&refreshFlg=Y&total_ls_inr="+total_ls_inr+"&ls_str="+lumpSum_str+
		"&sacStr="+sacStr+"&itemStr="+itemStr+"&qtyStr="+qtyStr+"&rateStr="+rateStr+"&amtStr="+amtStr+"&rowno="+rowno+"&lumpsumFlg="+lsFlg+"&chkBoxFlgStr="+chkBoxFlgStr+"&inr_mmbtu_str="+inr_mmbtu_str+"&qty_mmbtu_str="+qty_mmbtu_str;
// 		alert(url)
		location.replace(url);
	}else{
		document.getElementById('calcBase').value = "";
		alert(msg);
	}
}
...
// 		alert(document.getElementById('qty'+i).value+"--"+document.getElementById('rate'+i).value)
	...
	for(var i = 0 ; i < parseInt(tax_sz) ; i++){
		...
		totalTax+=(gross_amt * parseFloat(tax_structure))/100;
	}
	
	document.getElementById('totalTax').value = Math.round(totalTax);
	document.getElementById('netAmt').value = Math.round(gross_amt + totalTax);
...
// 	alert(value1)
	...
	var cont_st_dt = document.getElementById('cont_start_dt').value;
	var cont_end_dt = document.getElementById('cont_end_dt').value;
...
String refreshFlg = request.getParameter("refreshFlg")==null?"N":request.getParameter("refreshFlg");
...
String modFlg = request.getParameter("modFlg")==null?"N":request.getParameter("modFlg");
...
serviceInv.setCallFlag("fetchModifyInvDtl");
...
serviceInv.setRefreshFlg(refreshFlg);
...
String supp_nm = serviceInv.getSupp_nm();
...
String cust_nm = serviceInv.getCust_nm();

String plant_name = serviceInv.getPlant_nm();
...
inv_seq_no = serviceInv.getInv_seq_no();
new_inv_seq_no = serviceInv.getNew_inv_seq_no();
String hlpl_inv_seq_no = serviceInv.getInv_seq_no();
calcBase = serviceInv.getCalcBase();
tax_structure = serviceInv.getTax_structure();
...
String remark_1 = serviceInv.getRemark1();
String remark_2 = serviceInv.getRemark2();
calcBase = serviceInv.getCalcBase();
String sac_code = serviceInv.getSac_code();
String cont_start_dt = serviceInv.getCont_start_dt(); 
String cont_end_dt = serviceInv.getCont_end_dt();
	...
	<!-- /.Invoicing TAB START-->
		<div class="tab-pane active" id="invoicing">
			<form  method="post" action="../servlet/Frm_Service_Invoice" id = "SIform" >
				...
				<input type="hidden" name="option" value="Modify_Service_Invoice">
			...
			<input type="hidden" name="contSz"  id="contSz" value="1">
...
<!-- 			<input type="hidden" name="norows" value="1"> -->
			...
			<input type="hidden" name="contract_type" id="contract_type"  value="<%=cont_type%>">
...
<%-- 			<input type="hidden" name="alwInvGen" id="alwInvGen"  value="<%=alwInvGen%>"> --%>
			...
			<input type="hidden" name="cont_start_dt" id="cont_start_dt"  value="<%=cont_start_dt%>">
						...
						<div id="col-new">
							...
							<div class="box-header with-border" >
								<div class="form-group col-md-12 text-center">
							 	 	<label for="" ><h2> Service Invoice Modification</h2></label>
								</div>
								...
								<div class="table-responsive col-lg-6">
									<table class="table table-striped" >
										...
										<tr>
											...
											<td colspan="4">
												<input type="text" class="form-control form-control-sm" readonly="readonly"  name="supp_nm" id="supp_nm" value="<%=supp_nm %>" >
												<input type="hidden" class="form-control form-control-sm" readonly="readonly"  name="supp_cd" id="supp_cd" value="<%=supp_cd %>" >
											</td>
										</tr>
										<tr>
								...
								<div class="table-responsive col-lg-6">
									<table class="table table-striped" >
										...
										<tr>
											...
											<td colspan="4">
												<input type="text" class="form-control form-control-sm" id = "cust_nm"  name="cust_nm" value="<%=cust_nm %>"  readonly="readonly">
												<input type="hidden" class="form-control form-control-sm" id = "cust_cd"  name="cust_cd" value="<%=cust_cd %>"  readonly="readonly">
											</td>
										</tr>
 										<tr> 
 											<td class="info" colspan="2" >Buyer Plant</td> 
											<td colspan="4">
												<input type="text" class="form-control form-control-sm" id = "plant_name"  name="plant_name" value="<%=plant_name %>"  readonly="readonly">
												<input type="hidden" class="form-control form-control-sm" id = "cust_plant_cd"  name="cust_plant_cd" value="<%=cust_plant_cd %>"  readonly="readonly">
											</td>
										</tr> 
							...
							<div class="row">
								<div class="table-responsive col-lg-12">
									<table class="table table-striped" width="100%">
										<tr>
											<th colspan="12" class="text-center main-header">SN/LoA Invoice Details</th>
										</tr>
										<tr class="text-center info">
											...
											<th class="text-center">Invoice Quantity <br>(MMBTU) </th>
											<%if(!inr_mmbtu.equals("0")) {%>
												<th width="5%" class="text-center">Rate <br>(INR/MMBTU) </th>
												<th width="10%" class="text-center">Amount <br>(INR/MMBTU) </th>
											<%} %>
											<%if(!inr_km.equals("0")) {%>
												<th class="text-center">Kilometer(s)
													<i class="fa fa-clipboard" style="font-size:24px;color:red;vertical-align: middle;" title="Click here to Copy & Paste" onclick="copyPaste('<%=VInv_new_inv_seq_no.size()%>','km');"></i>
												</th>
												<th  width="5%" class="text-center">Rate <br>(INR/KM) </th>
												<th width="10%" class="text-center">Amount <br>(INR/KM) </th>
											<%} %>
										...
										<tr class="text-center">
											<td> <input type="checkbox" name="chkBox<%=i%>" id ="chkBox<%=i%>" onclick="enableDisable('<%=inr_km%>','<%=lumpsumFlg %>','<%=i%>','<%=VInv_new_inv_seq_no.size()%>','<%=inr_mmbtu %>');" 
											...
											<%if(!inr_km.equals("0")) {%>
												<td colspan="1" width="10%" style="background-color: #dde7ec;">
													<input class="form-control form-control-sm text-center" type="number" onkeyup="checkNum(this);" name="km" id="km<%=i%>" onchange="calcInrKm('<%=VInv_new_inv_seq_no.size()%>');" value="<%=Vinv_km.elementAt(i)%>" <%if(!Vinv_chkBox_Flg.elementAt(i).equals("Y")){ %>readonly="readonly"<%} %>>
												</td>
											...
											<%if(lumpsumFlg.equalsIgnoreCase("Y")){ %>
												<td colspan="1" width="10%" >
													<input class="form-control form-control-sm text-center" type="number" onkeyup="checkNum(this);" name="lumpsum" id="lumpsum<%=i%>" onchange="calcLump('<%=VInv_new_inv_seq_no.size()%>');" value="<%=Vinv_lumpSum.elementAt(i) %>" <%if(!Vinv_chkBox_Flg.elementAt(i).equals("Y")){ %>readonly="readonly"<%} %>>
												</td>
											<%} %>	
										...
										<tr>
											<td colspan="2"><input class="form-control form-control-sm" readonly="readonly" type="text" name="new_inv_seq_no" id="new_inv_seq_no" value="<%=new_inv_seq_no%>">  
											...
											<td colspan="2">
												<select class="form-control form-control-sm" onchange="setLineItems();" name="calcBase" id="calcBase">
											...
											<td colspan="2">
												<select class="form-control form-control-sm" name="tax_structure" id="tax_structure" onchange="setLineItems();">
												...
												<%if(!tax_structure.equals("") && !tax_structure.equals("0")){ %>
													<script type="text/javascript">
														document.getElementById('tax_structure').value = '<%=Double.parseDouble(tax_structure+"")%>';
													</script>					
												<%} %>
											</td>
										</tr>
										...
										<tr id="row<%=i%>">
											...
											<%if(i > 0) {%>
												<td colspan="1">
													<button type="button" class="btn btn-sm btn-danger btn-flat" onclick="removeRow('row<%=i%>');">
														<i class="fa fa-fw fa-minus-circle font-16"></i>
													</button>
												</td>
											<%}else{ %>
												<td colspan="1">&nbsp;</td>
					  			...
					  			<div class="table-responsive col-lg-6">
									<table class="table table-striped" >
										...
										<tr>
											...
											<td colspan="2">
												<input class="form-control form-control-sm text-right" type="text" name="totalTax" id="totalTax" readonly="readonly" value="<%=nf.format(Math.round(total_tax))%>">
											</td>
											<td colspan="2"></td>
										</tr>
								...
								<div class="table-responsive col-lg-12">
									<table class="table table-striped" >
										...
										<tr>	
											...
											<td class="col-lg-10">
												<textarea rows="3" cols="100" name="remark_2" id="remark_2"><%=remark_2 %></textarea>
											</td>
										</tr>
										<tr>
											<th colspan="11" class="text-right">
												<button type="button" class="btn btn-primary" name="verify" id="verify" value="Verify Before Submit" onclick="verifyInv('<%=cont_type%>','<%=new_inv_seq_no%>','<%=tax_name.size()%>');"> Verify Before Modify </button>
											</th>
											<th colspan="1" class="text-right">
												<button type="button" class="btn btn-success" name="" id="" title="Submit Inivoice Details" onclick="doSubmit();">Modify </button>
											</th>
										</tr>
									</table>
								</div>		
